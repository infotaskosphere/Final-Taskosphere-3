...
async function startSession(sessionId, webhookOnConnect = true, pairingPhone = null) {
  ...
  // pairing code must be requested AFTER the socket connects to WA servers
  // (i.e. after "connecting" state) but BEFORE any QR is generated.
  // We wait 3 seconds for the WS handshake to complete before requesting.
  if (pairingPhone && !state.creds?.registered) {
    setTimeout(async () => {
      try {
        if (sessions[sessionId]?.status === "connected") return; // already connected, skip
...
app.post("/sessions", async (req, res) => {
  ...
  const pairingPhone  = req.body.pairingPhone  || null;   // e.g. "919876543210"
  ...
  try {
    ...
    const mode = pairingPhone ? "pair-code" : "qr";
    res.json({ sessionId, status: "connecting", message: `Poll /sessions/:id/${mode}` });
  } catch (e) {
...
// Returns the 8-digit phone pairing code generated by requestPairingCode().
// The frontend polls this after starting a session in phone-pairing mode.
app.get("/sessions/:id/pair-code", (req, res) => {
  const s = sessions[req.params.id];
  if (!s) return res.status(404).json({ error: "Session not found" });
  if (s.status === "connected") {
    return res.json({ code: null, status: "connected" });
  }
  if (!s.pairCode) {
    // Code not yet ready — still requesting from WA servers
    return res.json({ code: null, status: s.status || "waiting", error: s.error || null });
  }
