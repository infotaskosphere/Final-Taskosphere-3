...
function SettingsTab({ isDark }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      load(); // refresh to pick up resolved folder name
    } catch (err) {
  ...
  return (
    <div className={`rounded-2xl border overflow-hidden shadow-sm ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
      ...
      <div className="p-5 space-y-5 max-w-lg">
        ...
        {/* Root Drive Folder — where every client's folder + subfolders get created */}
        <div className={`rounded-xl border p-4 ${isDark ? 'border-slate-700 bg-slate-900/30' : 'border-indigo-100 bg-indigo-50/40'}`}>
          <label className="text-xs font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-1.5 mb-1">
            <FolderOpen className="h-3.5 w-3.5" style={{ color: COLORS.deepBlue }} />
            Root Drive Folder
          </label>
          <p className="text-[10px] text-slate-400 mb-2">
            Paste the Google Drive folder link (or bare folder ID) where every client's folder — and its subfolders —
            should be created. This is the single source of truth used by Folder Architect, bulk creation, and the
            Documents panel; nothing about it is hard-coded.
          </p>
          <Input
          ...
          {rootFolderInfo.id && (
            <p className="text-[11px] text-emerald-600 dark:text-emerald-400 mt-2 flex items-center gap-1">
              ✓ Connected{rootFolderInfo.name ? `: “${rootFolderInfo.name}”` : ''} · ID {rootFolderInfo.id}
            </p>
          )}
          {!settings.root_drive_folder && (
            <p className="text-[11px] text-amber-600 dark:text-amber-400 mt-2">
              Not set — client folders will be created at the root of the connected Drive account until you set this.
            </p>
          )}
        </div>
