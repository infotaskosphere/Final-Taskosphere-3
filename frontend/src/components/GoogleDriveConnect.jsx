 ...
 * ──────────────────────────────────────────────────────────────────────────
 * Drop-in component that lets an admin connect / disconnect Google Drive
 * without touching any environment variables.
 *
 * HOW IT WORKS
 *   1. On mount → GET /auth/google/status  (shows connected / not connected)
 *   2. "Connect" button → window.location = BACKEND_URL/auth/google
 *      Google OAuth flow runs, backend callback saves refresh_token to DB,
 *      then redirects back to this page with ?drive=connected (or =error)
 *   3. On return, the URL param is read → toast shown → status re-fetched
 *   4. "Disconnect" button → POST /auth/google/disconnect
 *
 * USAGE
 *   import GoogleDriveConnect from '@/components/GoogleDriveConnect';
 *   <GoogleDriveConnect isDark={isDark} />
 *
 * Place this anywhere — inside GeneralSettings, a dedicated Integrations tab,
 * or the ClientPortalManagerPage settings section.
 */

import React, { useState, useEffect, useCallback } from 'react';
import { toast } from 'sonner';
import {
  HardDriveUpload, CheckCircle2, AlertCircle, Loader2,
  Unplug, RefreshCw, ExternalLink, Info, ShieldCheck,
} from 'lucide-react';
...
// ── Config ────────────────────────────────────────────────────────────────
const BACKEND_URL =
  (import.meta.env.VITE_API_URL || 'https://final-taskosphere-backend.onrender.com')
...
export default function GoogleDriveConnect({ isDark = false }) {
  const [status,       setStatus]       = useState(null);   // null | { connected, source }
  ...
  // ── On mount: read URL param & fetch status ───────────────────────────
  useEffect(() => {
    ...
    if (driveParam === 'connected') {
      toast.success('Google Drive connected successfully!', {
        description: 'Invoices can now be saved directly to your Drive.',
      });
  ...
  // ── Connect: redirect to backend OAuth ───────────────────────────────
  const handleConnect = () => {
  ...
  const handleDisconnect = async () => {
    if (!window.confirm('Disconnect Google Drive? Invoice uploads will stop working until you reconnect.')) return;
  ...
  // ── Styles ────────────────────────────────────────────────────────────
  const D = isDark;
  const card = `rounded-2xl border ${D ? 'bg-slate-800/60 border-slate-700/70' : 'bg-white border-slate-200'} shadow-sm overflow-hidden`;
      ...
      <div className="p-6">
        ...
        <div className="flex items-start justify-between gap-4 mb-5">
          <div className="flex items-center gap-3.5">
            {/* Google Drive icon (SVG inline — no external deps) */}
            <div
              className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm"
              style={{
                background: connected
                  ? 'linear-gradient(135deg,#e8f5e9,#c8e6c9)'
                  : 'linear-gradient(135deg,#e3f0fb,#c5dcf7)',
              }}
            >
              <svg viewBox="0 0 87.3 78" xmlns="http://www.w3.org/2000/svg" className="w-6 h-6">
            ...
            <div>
              <h3 className={`font-bold text-base ${D ? 'text-slate-100' : 'text-slate-800'}`}>
                Google Drive
              </h3>
          ...
          {!connected ? (
            ...
            >
              <svg viewBox="0 0 87.3 78" xmlns="http://www.w3.org/2000/svg" className="w-4 h-4">
                ...
                <path d="m43.65 25 13.75-23.8c-1.35-.8-2.9-1.2-4.5-1.2h-18.5c-1.6 0-3.15.45-4.5 1.2z" fill="#ffffff99"/>
                <path d="m59.8 53h-32.3l-13.75 23.8c1.35.8 2.9 1.2 4.5 1.2h50.8c1.6 0 3.15-.45 4.5-1.2z" fill="#ffffff99"/>
                <path d="m73.4 26.5-12.7-22c-.8-1.4-1.95-2.5-3.3-3.3l-13.75 23.8 16.15 27h27.45c0-1.55-.4-3.1-1.2-4.5z" fill="#ffffff99"/>
              </svg>
            ...
            <>
              <Button
                ...
                variant="outline"
                className={`h-9 px-4 rounded-xl text-sm gap-2 ${
                  D
        ...
        <p className={`text-[10px] mt-4 pt-4 border-t ${D ? 'border-slate-700 text-slate-500' : 'border-slate-100 text-slate-400'}`}>
          After connecting, set <strong>GOOGLE_REFRESH_TOKEN</strong> in your Render environment variables
          for persistence across server restarts. The token is also saved in the database for immediate use.
        </p>
      </div>
    </div>
  );
