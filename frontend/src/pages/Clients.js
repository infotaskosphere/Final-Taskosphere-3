...
export default function Clients() {
  ...
  const isClientVisibleToUser = (client) => {
    ...
    const isDirectlyAssigned = assignments.some(a => a.user_id === user.id) || legacyAssignedTo === user.id;
    if (isDirectlyAssigned) return true;

    // Check permission: can_view_all_clients
    if (canViewAllClients) return true;

    // Check assigned_clients permission list
    const permAssignedClients = user?.permissions?.assigned_clients || [];
    if (permAssignedClients.includes(client.id)) return true;

    // Check department-based visibility:
    // If user has departments, check if client has services matching user's departments
    const userDepts = user?.departments || [];
    if (userDepts.length > 0 && (client?.services || []).length > 0) {
      const relevantServices = userDepts.flatMap(dept => DEPT_TO_SERVICES[dept] || []);
      const clientServices = (client?.services || []).map(s => s.replace('Other: ', ''));
      const hasMatchingService = clientServices.some(cs =>
        relevantServices.some(rs => cs.toLowerCase().includes(rs.toLowerCase()) || rs.toLowerCase().includes(cs.toLowerCase()))
      );
