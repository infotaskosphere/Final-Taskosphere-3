...
export default function StaffActivity() {
  ...
  const canViewAttendanceReport = hasPermission("can_view_attendance"); // Fixed to match server
  // Removed unused canViewActivitySummary
  const canSendReminders = hasPermission("can_send_reminders");
  const canViewReports = hasPermission("can_view_reports"); // Use for task analytics as proxy
  ...
  // Polling intervals (45s refresh when tab active) - To reduce lag, make conditional
  useEffect(() => {
  ...
  // Simplified watermark hiding (run once per tab change)
  useEffect(() => {
    const hideWatermark = () => {
  ...
  // Memoize filtered data to reduce re-computations
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <motion.div variants={itemVariants}>
          <TabsList className="bg-white p-1 rounded-xl border border-slate-200 inline-flex">
            ...
            {(user.role === 'admin' || canViewReports) && ( // Fixed to use role or existing permission
        ...
        <TabsContent value="todos" className="mt-6">
          <Card className="border border-slate-200 shadow-sm p-4">
            ...
            ) : (user.role !== 'admin' && selectedUser !== user.id) ? ( // Fixed permission logic to match server (admin or own)
              <div className="text-center py-12 text-red-500">
                <AlertCircle className="h-12 w-12 mx-auto mb-3 text-red-300" />
                <p>Access denied: You can only view your own to-do list</p>
              </div>
            ) : (
        ...
        {/* Task Analytics Tab */}
        {(user.role === 'admin' || canViewReports) && ( // Fixed to use role or reports permission
          <TabsContent value="tasks">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card className="p-6 border border-slate-200"> {/* Added second card for priority */}
                <CardTitle className="text-lg mb-4">Priority Distribution</CardTitle>
                <div className="h-[300px]">
