...
export default function JournalEntries() {
  ...
  const del = async id => {
    ...
    try { await api.delete(`/accounting/journal-entries/${id}`); toast.success('Deleted'); fetchEntries(); }
      ...
      <div className={`${card} p-3 mb-4 flex flex-wrap items-center gap-3`}>
        ...
        <button onClick={async () => {
          try {
            ...
            if (filter.from) params.set('from_date', filter.from);
            if (filter.to)   params.set('to_date',   filter.to);
            const resp = await fetch(`/accounting/reports/export/journal-entries?${params}`, { headers: { Authorization: `Bearer ${token}` } });
