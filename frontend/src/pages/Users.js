...
const UserCard = ({ userData, onEdit, onDelete, onPermissions, onApprove, onReject, currentUserId, COLORS, isAdmin, canEditUsers, canManagePermissions, approving }) => {
  ...
  return (
    ...
    >
      ...
      {userDepts.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-4">{userDepts.map(dept => <DeptPill key={dept} dept={dept} size="sm" />)}</div>
      )}
...
export default function Users() {
  ...
  useEffect(() => { if (canViewUserPage) { fetchUsers(); fetchClients(); } }, [canViewUserPage]);
  ...
  const handleEdit = (userData) => {
    ...
    setFormData({
      ...
      role: userData.role, departments: userData.departments || [],
  ...
  const handleDelete = async (id) => {
    if (!canEditUsers) { toast.error('No permission to delete users'); return; }
    if (!window.confirm('Permanently delete this user and their logs?')) return;
    try {
  ...
  const handleApprove = async (userData) => {
    ...
    try {
      ...
      fetchUsers();
    } catch (err) { toast.error(err.response?.data?.detail || 'Failed to approve user'); }
  ...
  const handleReject = async (userData) => {
    ...
    try {
      ...
      fetchUsers();
    } catch (err) { toast.error(err.response?.data?.detail || 'Failed to reject user'); }
    finally { setApprovingId(null); }
  };
  ...
  const filteredUsers = users.filter(u => {
    ...
    if (activeTab === 'pending')  return matches && u.status === 'pending_approval';
    if (activeTab === 'rejected') return matches && u.status === 'rejected';
    if (activeTab === 'all')      return matches;
    return matches && u.role?.toLowerCase() === activeTab;
  });
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          {canEditUsers && (
            <DialogTrigger asChild>
              <Button className="rounded-xl font-bold h-12 shadow-lg transition-all hover:scale-105"
          ...
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl">
            ...
            <div className="space-y-6 py-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="space-y-2">
                  <Label className="font-semibold">Corporate Email</Label>
              ...
              <div className="bg-blue-50/50 p-5 rounded-2xl border border-blue-100 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {[
                    { label: 'Standard Punch-In', name: 'punch_in_time' },
                    { label: 'Grace Allowance (HH:MM)', name: 'grace_time' },
                    { label: 'Expected Punch-Out', name: 'punch_out_time' },
                  ].map(f => (
                    <div key={f.name} className="space-y-2">
                      <Label className="text-xs font-bold text-slate-600">{f.label}</Label>
                      <Input type="time" name={f.name} value={formData[f.name]} onChange={handleInputChange} className="rounded-lg bg-white" />
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="space-y-2">
                  ...
                  <Input type="number" name="telegram_id" value={formData.telegram_id || ''} onChange={handleInputChange} placeholder="Numeric Telegram ID" className="rounded-xl" />
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-2">
                  <Label className="font-semibold">Primary System Role</Label>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="bg-white p-1 rounded-2xl border shadow-sm self-start">
          <TabsList className="bg-transparent h-10 flex flex-wrap gap-0.5">
            {['all', 'admin', 'manager', 'staff'].map(t => (
              <TabsTrigger key={t} value={t} className="rounded-xl font-bold px-4 capitalize">{t === 'all' ? 'All' : t + 's'}</TabsTrigger>
            ))}
            {isAdmin && (
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto rounded-3xl p-0 border-none shadow-2xl">
          <div className="sticky top-0 z-10 p-6 bg-white border-b flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-emerald-50 rounded-2xl text-emerald-600"><Shield className="h-7 w-7" /></div>
              <div>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-2xl">
              <span className="text-sm text-amber-800 font-semibold flex-1">
                Role: <b className="capitalize">{selectedUserForPermissions?.role}</b> — reset all toggles to role defaults?
              </span>
              <button type="button"
                onClick={() => {
              ...
              <AccordionItem value="global" className="border rounded-2xl px-4 overflow-hidden shadow-sm">
                ...
                <AccordionContent className="pb-4 space-y-2">
                  {[
                    ...
                    { key: 'can_view_all_leads',              label: 'Leads Pipeline Access',         desc: 'View the global leads dashboard' },
                    { key: 'can_view_user_page',              label: 'User Directory Access',         desc: 'View the team members directory' },
                    ...
                    <div key={p.key} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl hover:bg-slate-100 transition-colors">
                      ...
                      <Switch checked={!!permissions[p.key]} onCheckedChange={val => setPermissions({ ...permissions, [p.key]: val })} />
                    </div>
                  ))}
                </AccordionContent>
              </AccordionItem>
                          ...
                          return (
                            <Badge key={u.id} onClick={() =>
                              ...
                              className={`cursor-pointer px-3 py-1.5 rounded-lg border-2 transition-all ${
                                sel ? "bg-emerald-500 border-emerald-600 text-white shadow-md scale-105"
              ...
              <AccordionItem value="clients" className="border rounded-2xl px-4 overflow-hidden shadow-sm">
                ...
                <AccordionContent className="pb-4 space-y-4">
                  <div className="relative">
                    ...
                    <Input placeholder="Filter company list..."
