...
import {
  ...
  Clock, UserCheck, UserX, AlertCircle, Fingerprint,
} from 'lucide-react';
...
const DeptPill = ({ dept, size = 'sm' }) => {
  ...
  return (
    <span
      ...
      style={{ background: `${deptInfo.color}15`, color: deptInfo.color, border: `1px solid ${deptInfo.color}30` }}
    >
      {deptInfo.label}
    </span>
  );
...
const PendingUserCard = ({ userData, onApprove, onReject, approving }) => (
  <motion.div variants={itemVariants} layout
    ...
    <div className="flex gap-2 pt-3 border-t border-amber-100">
      ...
      <Button size="sm" disabled={approving === userData.id} onClick={() => onReject(userData)} variant="outline"
...
const UserCard = ({
  ...
  currentUserId, COLORS, isAdmin, canEditUsers, canManagePermissions, approving,
}) => {
  ...
  return (
    ...
    >
      ...
      <div className={`absolute top-3 right-3 flex gap-1 transition-all duration-200 ${showActions ? 'opacity-100' : 'opacity-0'}`}>
        {canManagePermissions && userData.role !== 'admin' && !isPending && (
          <button onClick={() => onPermissions(userData)}
            className="p-2 rounded-lg bg-emerald-50 hover:bg-emerald-100 text-emerald-600 transition-colors" title="Permissions">
            <Shield className="h-4 w-4" />
          </button>
        )}
        {canEditUsers && !isPending && (
          <button onClick={() => onEdit(userData)}
            className="p-2 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-600 transition-colors" title="Edit">
            <Edit className="h-4 w-4" />
          </button>
        )}
        {canEditUsers && userData.id !== currentUserId && (
          <button onClick={() => onDelete(userData.id)}
            className="p-2 rounded-lg bg-red-50 hover:bg-red-100 text-red-600 transition-colors" title="Delete">
      ...
      <div className="flex items-start gap-3 sm:gap-4 mb-4">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-base sm:text-lg font-outfit font-semibold text-slate-900 truncate">{userData.full_name}</h3>
      ...
      <div className="space-y-2 text-xs sm:text-sm text-slate-600">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1">
          {userData.punch_in_time  && <p className="flex items-center gap-1 text-[10px] text-slate-500">In:  {userData.punch_in_time}</p>}
          {userData.punch_out_time && <p className="flex items-center gap-1 text-[10px] text-slate-500">Out: {userData.punch_out_time}</p>}
        </div>
        <p className="flex items-center gap-2"><Calendar className="h-3.5 w-3.5 flex-shrink-0" />
          Joined {userData.created_at ? format(new Date(userData.created_at), 'MMM dd, yyyy') : 'N/A'}
        </p>

        {/* FIX: Show machine_employee_id + sync status on card so admins
            can see which device ID is assigned without clicking Edit. */}
        {userData.machine_employee_id && (
          <p className="flex items-center gap-2 text-[10px] text-violet-600 font-semibold pt-1 border-t border-slate-100 mt-1">
            <Fingerprint className="h-3.5 w-3.5 flex-shrink-0" />
            Machine ID: {userData.machine_employee_id}
            {userData.machine_synced
              ? <span className="text-emerald-600 font-bold">✓ Synced</span>
              : <span className="text-amber-500 font-bold">⏳ Not synced</span>}
          </p>
        )}
      </div>
...
export default function Users() {
  ...
  const [formData, setFormData] = useState({
    ...
    punch_in_time: '', grace_time: '', punch_out_time: '',
    telegram_id: null,
    // machine_employee_id is intentionally excluded from this generic form payload.
    // It is updated separately via PUT /users/{id}/machine-id to enforce uniqueness.
    // We keep it in a separate local state field below.
    is_active: true,
  });

  // FIX: Separate state for machine_employee_id so it is NEVER included in the
  // generic PUT /users/{id} payload. It is submitted via the dedicated endpoint
  // PUT /users/{id}/machine-id which performs the uniqueness check.
  const [machineEmployeeId, setMachineEmployeeId]         = useState('');
  const [originalMachineEmployeeId, setOriginalMachineEmployeeId] = useState('');
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (selectedUser) {
        // ── Edit path ─────────────────────────────────────────────────────
        // Build generic update payload — machine_employee_id deliberately excluded.
        const updatePayload = {
        ...
        // FIX: Only call the dedicated machine-id endpoint when the value
        // has actually changed, and only for admins.
        // This endpoint (PUT /users/{id}/machine-id) performs the uniqueness
        // check that the generic PUT endpoint skips.
        if (isAdmin && machineEmployeeId !== originalMachineEmployeeId) {
          try {
            await api.put(`/users/${selectedUser.id}/machine-id`, {
              machine_employee_id: machineEmployeeId || null,
            });
          } catch (machineErr) {
            // Surface the uniqueness error specifically
            const detail = machineErr.response?.data?.detail;
            toast.error(
              typeof detail === 'string'
                ? `Machine ID: ${detail}`
                : 'Failed to update Machine Employee ID'
            );
            // Don't close dialog — let admin fix the conflict
            setLoading(false);
            return;
          }
        }
        ...
        // ── Create path ───────────────────────────────────────────────────
        // machine_employee_id is not set at creation time —
        // it should be assigned after the user is approved and on the device.
  ...
  const handleEdit = (userData) => {
    ...
    // FIX: track machine_employee_id separately so we know when it changed
    const mid = userData.machine_employee_id || '';
    setMachineEmployeeId(mid);
    setOriginalMachineEmployeeId(mid);
    setDialogOpen(true);
  };
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl">
            ...
            <div className="space-y-6 py-4">
              ...
              {/* FIX: Machine Employee ID — edit mode + admin only.
                  Uses its own state (not inside formData) so it is submitted via
                  PUT /users/{id}/machine-id (uniqueness-checked) rather than through
                  the generic PUT /users/{id} which skips the uniqueness check. */}
              {isAdmin && selectedUser && (
                <div className="bg-violet-50 border border-violet-200 p-4 rounded-2xl space-y-2">
                  ...
                  <p className="text-[11px] text-violet-500 font-medium">
                    Numeric UID on the eSSL/ZKTeco device. Leave blank to unassign.
                    {machineEmployeeId !== originalMachineEmployeeId && (
                      <span className="ml-1 text-amber-600">⚠ Changed — will be verified for uniqueness on save.</span>
                    )}
                  </p>
                  {selectedUser?.machine_synced !== undefined && (
                    <p className={`text-[11px] font-bold flex items-center gap-1 ${selectedUser.machine_synced ? 'text-emerald-600' : 'text-amber-500'}`}>
                      {selectedUser.machine_synced ? '✓ Synced to device' : '⏳ Not yet synced to device'}
                    </p>
                  )}
                </div>
              )}
                ...
                <div className="space-y-2">
                  ...
                  {roleChanged && selectedUser && (
                    <div className="mt-2 flex items-center gap-3 p-3 bg-amber-50 border border-amber-200 rounded-xl">
                      ...
                      <button type="button" onClick={handleResetPermissionsToRole}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
                  {DEPARTMENTS.map(dept => {
                    ...
                    return (
                      <button key={dept.value} onClick={() => handleDepartmentChange(dept.value)}
          ...
          {filteredUsers.length === 0 ? (
            ...
            filteredUsers.map(userData => (
              <UserCard key={userData.id} userData={userData}
                ...
                currentUserId={user?.id} COLORS={COLORS} isAdmin={isAdmin}
                  ...
                  {[
                    ...
                    <div key={section.key} className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2 p-3 bg-slate-50 rounded-2xl min-h-[50px]">
                        {users.filter(u => u.id !== selectedUserForPermissions?.id).map(u => {
              ...
              <AccordionItem value="clients" className="border rounded-2xl px-4 overflow-hidden shadow-sm">
                ...
                <AccordionContent className="pb-4 space-y-4">
                  <div className="relative">
                    ...
                    <Input placeholder="Filter company list..." value={clientSearchQuery}
                      onChange={e => setClientSearchQuery(e.target.value)} className="pl-10 h-10 rounded-xl" />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                    {clients.filter(c => c.company_name.toLowerCase().includes(clientSearchQuery.toLowerCase())).map(client => {
