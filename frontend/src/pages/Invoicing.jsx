...
// ─── Brand Colors (matching existing app) ─────────────────────────────────────
const COLORS = {
  deepBlue:     '#0D3B66',
  mediumBlue:   '#1F6FB2',
  emeraldGreen: '#1FAF5A',
...
const computeTotals = (items, isInter, discAmt = 0, shipping = 0, other = 0) => {
  ...
  return { items: comp, subtotal: Math.round(subtotal * 100) / 100,
    total_discount: Math.round(totDisc * 100) / 100,
    total_taxable: Math.round(totTax * 100) / 100,
    total_cgst: Math.round(totCGST * 100) / 100, total_sgst: Math.round(totSGST * 100) / 100,
    total_igst: Math.round(totIGST * 100) / 100, total_gst: totGST, grand_total: grand };
};
...
const StatCard = ({ label, value, sub, icon: Icon, color, bg, onClick, isDark, trend }) => (
  <div onClick={onClick}
    ...
    <p className={`text-[10px] font-bold uppercase tracking-widest mb-1 pl-2 ${isDark ? 'text-slate-400' : 'text-slate-400'}`}>{label}</p>
...
// ════════════════════════════════════════════════════════════════════════════════
// MINI REVENUE CHART (pure SVG, no chart lib needed)
// ════════════════════════════════════════════════════════════════════════════════
...
const PaymentModal = ({ invoice, open, onClose, onSuccess, isDark }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/payments', { invoice_id: invoice.id, amount: parseFloat(form.amount),
        ...
        <div className="px-6 py-5" style={{ background: `linear-gradient(135deg, ${COLORS.deepBlue}, ${COLORS.mediumBlue})` }}>
          ...
          <div className="mt-4 flex gap-4">
            <div className="flex-1 bg-white/10 rounded-xl px-3 py-2">
              <p className="text-white/50 text-[9px] uppercase tracking-wider">Invoice Total</p>
              <p className="text-white font-bold text-sm">{fmtC(invoice.grand_total)}</p>
            </div>
            <div className="flex-1 bg-white/10 rounded-xl px-3 py-2">
              <p className="text-white/50 text-[9px] uppercase tracking-wider">Paid So Far</p>
              <p className="text-emerald-300 font-bold text-sm">{fmtC(invoice.amount_paid)}</p>
            </div>
            <div className="flex-1 bg-white/10 rounded-xl px-3 py-2">
              <p className="text-white/50 text-[9px] uppercase tracking-wider">Balance Due</p>
              <p className="text-amber-300 font-bold text-sm">{fmtC(invoice.amount_due)}</p>
            </div>
          </div>
        </div>
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <Input type="number" step="0.01" min="0.01" className="pl-8 h-11 rounded-xl border-slate-200 focus:border-blue-400" value={form.amount} onChange={e => setForm(p => ({ ...p, amount: e.target.value }))} required />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1.5 block">Payment Date *</label>
              <Input type="date" className="h-11 rounded-xl border-slate-200 focus:border-blue-400" value={form.payment_date} onChange={e => setForm(p => ({ ...p, payment_date: e.target.value }))} required />
            </div>
            <div>
          ...
          <div>
            ...
            <Input className="h-11 rounded-xl border-slate-200 focus:border-blue-400" placeholder="Transaction / cheque reference" value={form.reference_no} onChange={e => setForm(p => ({ ...p, reference_no: e.target.value }))} />
          </div>
          <div>
            <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1.5 block">Notes</label>
            <Textarea className="rounded-xl border-slate-200 text-sm min-h-[70px] resize-none" value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} />
          </div>
...
const InvoiceForm = ({ open, onClose, editingInv, companies, clients, leads, onSuccess, isDark }) => {
  ...
  const fillFromClient = useCallback((clientId) => {
    const c = clients.find(x => x.id === clientId);
    if (!c) return;
    setForm(p => ({ ...p, client_id: clientId,
      client_name: c.company_name || '', client_email: c.email || '',
      client_phone: c.phone || '', client_address: [c.address, c.city, c.state].filter(Boolean).join(', '),
                ...
                <div className={sectionCls}>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <Select value={form.lead_id || ''} onValueChange={v => setField('lead_id', v || null)}>
                      ...
                      <div key={idx} className={`border rounded-xl p-4 relative ${isDark ? 'bg-slate-800 border-slate-600' : 'bg-white border-slate-200'}`}>
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">
                            ...
                            <Select value={item.product_id || ''} onValueChange={v => v && fillFromProduct(idx, v)}>
                              <SelectTrigger className="h-7 w-44 text-xs rounded-lg border-slate-200">
              ...
              <div className="space-y-5">
                <div className={sectionCls}>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div>
                      ...
                      <Input type="number" min="0" step="0.01" className={inputCls} value={form.discount_amount} onChange={e => setField('discount_amount', parseFloat(e.target.value) || 0)} />
                    </div>
                    <div>
                      <label className={labelCls}>Shipping Charges (₹)</label>
                      <Input type="number" min="0" step="0.01" className={inputCls} value={form.shipping_charges} onChange={e => setField('shipping_charges', parseFloat(e.target.value) || 0)} />
                    </div>
                    <div>
                      <label className={labelCls}>Other Charges (₹)</label>
                      <Input type="number" min="0" step="0.01" className={inputCls} value={form.other_charges} onChange={e => setField('other_charges', parseFloat(e.target.value) || 0)} />
                    </div>
                  </div>
                </div>
                ...
                <div className={`border rounded-2xl overflow-hidden ${isDark ? 'border-slate-700' : 'border-slate-200'}`}>
                  {[
                    ...
                    ['Taxable Value', totals.total_taxable, false],
                    form.is_interstate
                      ? ['IGST', totals.total_igst, false]
                      : null,
                    !form.is_interstate ? ['CGST', totals.total_cgst, false] : null,
                    !form.is_interstate ? ['SGST', totals.total_sgst, false] : null,
                    form.shipping_charges > 0 ? ['Shipping Charges', form.shipping_charges, false] : null,
                    form.other_charges > 0 ? ['Other Charges', form.other_charges, false] : null,
                    ...
                    <div key={label} className={`flex items-center justify-between px-5 py-3 border-b last:border-0 ${bold ? `${isDark ? 'bg-slate-700' : 'bg-slate-50'}` : ''} ${isDark ? 'border-slate-700' : 'border-slate-100'}`}>
                      ...
                      <span className={`text-sm ${bold ? 'text-xl font-black' : 'font-semibold'} ${neg ? 'text-red-500' : bold ? (isDark ? 'text-blue-400' : COLORS.deepBlue) : (isDark ? 'text-slate-200' : 'text-slate-800')}`}
                ...
                <div className={sectionCls}>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Textarea className={`rounded-xl text-sm min-h-[80px] resize-none ${isDark ? 'bg-slate-700 border-slate-600 text-slate-100' : 'bg-white border-slate-200'}`} value={form.notes} onChange={e => setField('notes', e.target.value)} />
                    </div>
                    <div>
                      <label className={labelCls}>Terms & Conditions</label>
                      <Textarea className={`rounded-xl text-sm min-h-[80px] resize-none ${isDark ? 'bg-slate-700 border-slate-600 text-slate-100' : 'bg-white border-slate-200'}`} value={form.terms_conditions} onChange={e => setField('terms_conditions', e.target.value)} />
                    </div>
                  </div>
                </div>
                <div className={sectionCls}>
...
// ════════════════════════════════════════════════════════════════════════════════
// INVOICE DETAIL PANEL (side drawer style)
// ════════════════════════════════════════════════════════════════════════════════
      ...
      <DialogContent className={`max-w-2xl max-h-[92vh] overflow-hidden flex flex-col rounded-2xl border shadow-2xl p-0 ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200'}`}>
        ...
        <div className="px-7 py-5 relative overflow-hidden flex-shrink-0" style={{ background: `linear-gradient(135deg, ${COLORS.deepBlue}, ${COLORS.mediumBlue})` }}>
          ...
          <div className="relative mt-4 grid grid-cols-3 gap-3">
            {[
              ['Invoice Total', invoice.grand_total, 'text-white'],
              ['Amount Paid',   invoice.amount_paid,  'text-emerald-300'],
              ['Balance Due',   invoice.amount_due,   'text-amber-300'],
            ].map(([label, val, cls]) => (
                    ...
                    {(invoice.items || []).map((it, i) => (
                      <tr key={i} className={`border-t ${isDark ? 'border-slate-700 hover:bg-slate-700/20' : 'border-slate-100 hover:bg-slate-50'}`}>
                        ...
                        <td className="px-3 py-2.5 text-amber-600 font-medium">
                          {isInterstate ? fmtC(it.igst_amount) : fmtC(it.cgst_amount + it.sgst_amount)}
                        </td>
        ...
        <div className={`flex-shrink-0 flex items-center gap-2 px-7 py-4 border-t flex-wrap ${isDark ? 'border-slate-700 bg-slate-800' : 'border-slate-100 bg-white'}`}>
          <Button variant="outline" size="sm" onClick={() => { onClose(); onEdit?.(invoice); }} className="rounded-xl text-xs h-9 gap-1.5">
            <Edit className="h-3.5 w-3.5" /> Edit
          </Button>
          <Button variant="outline" size="sm" onClick={() => onDownloadPdf?.(invoice)} className="rounded-xl text-xs h-9 gap-1.5">
          ...
          <div className={`w-72 flex-shrink-0 p-5 border-r overflow-y-auto ${isDark ? 'border-slate-700 bg-slate-800' : 'border-slate-100 bg-slate-50/40'}`}>
            ...
            <form onSubmit={handleSave} className="space-y-3">
              ...
              <div className="flex gap-2">
                <Button type="submit" disabled={loading} size="sm" className="flex-1 h-9 rounded-xl text-white text-xs font-semibold" style={{ background: `linear-gradient(135deg, ${COLORS.deepBlue}, ${COLORS.mediumBlue})` }}>
          ...
          <div className="flex-1 overflow-y-auto">
            {products.length === 0 ? (
              ...
              <div key={p.id} className={`flex items-center gap-3 px-5 py-3.5 border-b group transition-colors ${isDark ? 'border-slate-700 hover:bg-slate-700/30' : 'border-slate-100 hover:bg-slate-50'}`}>
                <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 text-white text-xs font-bold" style={{ background: p.is_service ? `linear-gradient(135deg, ${COLORS.deepBlue}, ${COLORS.mediumBlue})` : `linear-gradient(135deg, #065f46, #059669)` }}>
                  {p.is_service ? 'S' : 'P'}
                </div>
...
export default function Invoicing() {
  ...
  const handleExport = useCallback(() => {
    ...
    const rows = [
      ...
      ...enrichedFiltered.map(inv => [
        ...
        inv.client_name, inv.invoice_date, inv.due_date,
        inv.total_taxable, inv.total_gst, inv.grand_total,
  ...
  const handleConvertFromQuotation = useCallback(async (quotationId) => {
    try {
      const r = await api.post(`/invoices/from-quotation/${quotationId}`);
      toast.success('Quotation converted to invoice!');
      fetchAll();
      setEditingInv(r.data); setFormOpen(true);
    } catch { toast.error('Conversion failed'); }
  }, [fetchAll]);

  const metricCardCls = `rounded-2xl border p-5 relative overflow-hidden hover:-translate-y-0.5 hover:shadow-lg transition-all ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200/80'}`;
    ...
    <div className={`min-h-screen p-5 md:p-7 space-y-5 ${isDark ? 'bg-slate-900' : 'bg-slate-50'}`}>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard label="Total Revenue" value={fmtC(stats.total_revenue)} sub={`${stats.total_invoices} invoices`}
            icon={IndianRupee} color={COLORS.mediumBlue} bg={`${COLORS.mediumBlue}12`}
            isDark={isDark} onClick={() => setStatusFilter('all')} />
          <StatCard label="Outstanding" value={fmtC(stats.total_outstanding)} sub={`${stats.overdue_count} overdue`}
            icon={AlertCircle} color={COLORS.coral} bg={`${COLORS.coral}15`}
            isDark={isDark} onClick={() => setStatusFilter('overdue')} />
          <StatCard label="This Month" value={fmtC(stats.month_revenue)} sub={`${stats.month_invoices} invoices · ₹${fmt(stats.month_collected)} collected`}
            icon={TrendingUp} color={COLORS.emeraldGreen} bg={`${COLORS.emeraldGreen}12`}
            isDark={isDark} />
          <StatCard label="Total GST" value={fmtC(stats.total_gst)} sub={`${stats.paid_count} paid · ${stats.draft_count} draft`}
            icon={Shield} color={COLORS.amber} bg={`${COLORS.amber}12`}
            isDark={isDark} />
        </div>
      )}
      ...
      <div className={`rounded-2xl border shadow-sm overflow-hidden ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-200/80'}`}>
        ...
        <div className={`grid border-b px-5 py-3 ${isDark ? 'bg-slate-700/50 border-slate-700' : 'bg-slate-50 border-slate-100'}`}
          ...
          {['Invoice No','Client','Date','Total','Paid','Balance','Status','Actions'].map(h => (
            <div key={h} className={`text-[10px] font-bold uppercase tracking-widest ${isDark ? 'text-slate-400' : 'text-slate-400'}`}>{h}</div>
          ))}
        </div>
              ...
              return (
                <div key={inv.id}
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
                    <button onClick={() => handleDownloadPdf(inv)} className="w-7 h-7 flex items-center justify-center rounded-lg text-slate-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/30 transition-colors" title="Download PDF"><Download className="h-3.5 w-3.5" /></button>
                    {inv.amount_due > 0 && (
                      <button onClick={() => { setPayInv(inv); setPayOpen(true); }} className="w-7 h-7 flex items-center justify-center rounded-lg text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 transition-colors" title="Record Payment"><IndianRupee className="h-3.5 w-3.5" /></button>
                    )}
                    {inv.status === 'draft' && (
          ...
          <div className={`flex items-center justify-between px-5 py-3 border-t ${isDark ? 'border-slate-700 bg-slate-800/50' : 'border-slate-100 bg-slate-50/50'}`}>
            <div className="flex items-center gap-6 text-xs">
              ...
              <span className="font-semibold text-emerald-600">
                Total: {fmtC(enrichedFiltered.reduce((s, i) => s + i.grand_total, 0))}
              </span>
              <span className="font-semibold text-amber-600">
                Outstanding: {fmtC(enrichedFiltered.reduce((s, i) => s + (i.amount_due || 0), 0))}
              </span>
              <span className="font-semibold" style={{ color: COLORS.mediumBlue }}>
                GST: {fmtC(enrichedFiltered.reduce((s, i) => s + (i.total_gst || 0), 0))}
              </span>
            </div>
          </div>
        )}
