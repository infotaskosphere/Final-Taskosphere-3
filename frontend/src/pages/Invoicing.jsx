...
def _find_amount_by_labels(text: str, labels: list[str]) -> float:
    """Find the amount that belongs to a label — the number immediately
    after it on the SAME line (or, failing that, on the very next line).

    Previous behaviour scanned a wide 180-character window after the first
    label match and took the MAX number found anywhere in it. On real
    invoices that window routinely spans past unrelated numbers (the next
    line item's HSN code, a discount line, a different total further down),
    so it would confidently return the wrong figure — e.g. reading the
    invoice's grand total as the GST amount, or an HSN/SAC code as the
    grand total. Restricting the search to the label's own line removes
    that failure mode; multi-line table headers (where a label's text is
    split across lines, e.g. "...IGST Total" / "Value Rate Amount...") no
    longer falsely match either, since matching is now per-line.
    """
    lines = text.split("\n")
    for label in labels:
        ...
        for i, line in enumerate(lines):
            ...
            if not m:
                continue
            found = _MONEY_RE.search(line[m.end():])
            if found:
                val = _money_to_float(found.group(1))
                if val > 0:
                    return val
            if i + 1 < len(lines):
                found = _MONEY_RE.search(lines[i + 1])
                if found:
                    val = _money_to_float(found.group(1))
                    if val > 0:
                        return val
...
def _parse_purchase_invoice_text(text: str, filename: str = "") -> dict:
    ...
    if not grand_total:
        # Real invoice totals almost always carry paise (two decimal places).
        # Bare integers in the body of an invoice are usually HSN/SAC codes,
        # quantities, or pincodes — so prefer decimal-valued numbers first,
        # and only fall back to bare integers if none exist.
        decimal_amounts = [_money_to_float(x) for x in re.findall(r"[0-9][0-9,]*\.\d{2}\b", compact)]
        decimal_amounts = [a for a in decimal_amounts if 1 <= a <= 10_00_00_000]
        if decimal_amounts:
            grand_total = max(decimal_amounts)
        else:
...
def _totals_reconcile(p: dict) -> bool:
    """True when grand_total is internally consistent with taxable + GST
    (within a small rounding tolerance), or when there's nothing to cross
    check against. Used to decide whether the regex parse can be trusted
    as-is or needs an AI cross-check."""
    gt = p.get("grand_total") or 0
    if not gt:
        return False
    ta, gst = p.get("taxable_amount") or 0, p.get("total_gst") or 0
    if ta and gst:
        return abs(gt - (ta + gst)) <= max(2.0, gt * 0.02)
    return True
...
async def _extract_purchase_invoice(contents: bytes, filename: str, content_type: str = "") -> dict:
    ...
    regex_parsed = _parse_purchase_invoice_text(text, filename) if text.strip() else {}
    reconciled = _totals_reconcile(regex_parsed)

    # Run the AI (vision) reading whenever the regex pass is missing key
    # fields OR its own totals don't add up — not just when fields are
    # entirely absent. A GSTIN being found is not proof the amounts are
    # right, so "we found *something*" is no longer good enough on its own.
    needs_ocr = (
        not regex_parsed
        or not regex_parsed.get("grand_total")
        or (not regex_parsed.get("invoice_no") and not regex_parsed.get("all_gstins"))
        or not reconciled
    )
    ...
    if regex_parsed and reconciled:
        # Regex pass is trustworthy — just fill gaps from AI if it ran.
        parsed = dict(regex_parsed)
        for k, v in ai_parsed.items():
            if not parsed.get(k) and v not in (None, "", [], 0):
                parsed[k] = v
    elif ai_parsed and _totals_reconcile(ai_parsed):
        # AI reading reconciles where the regex one didn't — trust its
        # numeric fields, but keep any text fields the regex pass already
        # found (e.g. a clean invoice_no) if the AI came back empty there.
        parsed = {**regex_parsed, **{k: v for k, v in ai_parsed.items() if v not in (None, "", [], 0)}}
    elif regex_parsed:
        parsed = regex_parsed
        for k, v in ai_parsed.items():
            if not parsed.get(k) and v not in (None, "", [], 0):
        ...
        parsed = ai_parsed or {"file_name": filename, "raw_text_excerpt": text[:5000]}
    ...
    parsed["needs_amount_review"] = not _totals_reconcile(parsed)
    return parsed
...
async def upload_purchase_invoice(file: UploadFile = File(...), client_id: Optional[str] = Form(None), company_id: Optional[str] = Form(None), current_user: User = Depends(get_current_user)):
    ...
    doc = {
        ...
        "needs_amount_review": bool(parsed.get("needs_amount_review")),
