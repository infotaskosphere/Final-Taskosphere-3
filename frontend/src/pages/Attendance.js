...
export default function Attendance() {
  ...
  // FIX: Poll every 5 minutes so machine-synced punches become visible
  // without a manual page refresh. The sync engine runs up to every 300s
  // server-side, so polling at the same interval keeps the UI in sync.
  useEffect(() => {
    const poll = setInterval(() => fetchData(), 5 * 60 * 1000);
  ...
  const handleExportPDF = useCallback(() => {
    ...
    doc.text('SOURCE',   155, 98);
    doc.text('LOCATION', 175, 98);
    ...
    attendanceHistory.slice(0, 15).forEach((record, index) => {
      ...
      const locLabel = locKey
        ...
        : '—';
      doc.text(record.source === 'machine' ? 'Biometric' : 'Web', 155, y);
      doc.text(locLabel.substring(0, 20), 175, y);
      y += 10;
    });
    doc.setFontSize(8);
                  ...
                  {selectedAttendance?.punch_in ? (
                    ...
                    >
                      ...
                      <div className="space-y-3 text-sm">
                        ...
                        {selectedAttendance.source === 'machine' && (
                          <div className="flex justify-between items-center">
                            <span className="text-slate-600 font-medium">Source</span>
                            <span className="text-xs font-bold text-violet-700 uppercase px-2 py-1 bg-violet-100 rounded flex items-center gap-1">
                        ...
                        >
                          <div className="flex justify-between items-start gap-3">
                            <div className="flex-1 min-w-0">
                              ...
                              {/* FIX: Show biometric source badge when punch came from device */}
                              {record.source === 'machine' && (
                                <p className="flex items-center gap-1 mt-1">
                                  <Fingerprint className="w-3 h-3 text-violet-500 flex-shrink-0" />
                                  <span className="text-[10px] font-bold text-violet-600">Biometric punch</span>
                                  {record.machine_punch_type && (
                                    <span className="text-[10px] text-violet-400">· {record.machine_punch_type}</span>
                                  )}
                                </p>
                              )}
                              {/* FIX 4: Location rows */}
