...
/**
 * Component for rendering department pills/badges
 */
...
/**
 * Component for rendering user status badges
 */
const StatusBadge = ({ status, isActive }) => {
  ...
  return (
    <span 
      className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border text-[10px] font-bold ${cfg.cls}`}
    >
      <span 
        className={`w-1.5 h-1.5 rounded-full ${cfg.dot} ${resolved === 'active' ? 'animate-pulse' : ''}`} 
      />
      {cfg.label}
    </span>
  );
...
/**
 * Component for rendering module access badges on user cards
 */
        ...
        return (
          <span 
            ...
            className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[9px] font-bold border ${
              b.active ? b.activeStyle : b.inactiveStyle
            }`}
          >
...
const PendingUserCard = ({ 
  ...
  <motion.div 
    ...
    layout
    className="relative bg-white dark:bg-slate-800 rounded-3xl overflow-hidden border border-amber-200 shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 hover:scale-[1.01]"
  >
    {/* Top stripe */}
...
const UserCard = ({ 
  ...
  return (
    <motion.div 
      variants={itemVariants} 
      layout
      className={`group relative bg-white dark:bg-slate-800 rounded-3xl overflow-hidden border transition-all duration-300 hover:-translate-y-1 hover:scale-[1.01] ${
        isPending
          ? 'border-amber-300 shadow-amber-100/50 shadow-md hover:shadow-xl'
          : hovered
          ? 'border-blue-200 shadow-xl shadow-blue-100/30'
          ...
          >
            {canManagePermissions && userData.role !== 'admin' && !isPending && (
              <button 
                ...
                className="w-8 h-8 rounded-2xl bg-emerald-50 hover:bg-emerald-100 text-emerald-600 flex items-center justify-center shadow-sm border border-emerald-200 transition-all hover:shadow-md"
            ...
            {(isAdmin || (canEditUsers && !isPending)) && (
              <button 
                ...
                className="w-8 h-8 rounded-2xl bg-blue-50 hover:bg-blue-100 text-blue-600 flex items-center justify-center shadow-sm border border-blue-200 transition-all hover:shadow-md"
                title="Edit"
              >
            ...
            {(isAdmin || canEditUsers) && userData.id !== currentUserId && (
              <button 
                ...
                className="w-8 h-8 rounded-2xl bg-red-50 hover:bg-red-100 text-red-600 flex items-center justify-center shadow-sm border border-red-200 transition-all hover:shadow-md"
                title="Delete"
              >
        ...
        <div className="flex items-start gap-3.5 mb-4">
          <div className="relative flex-shrink-0">
            <div className="w-[52px] h-[52px] rounded-2xl overflow-hidden shadow-sm">
              {userData.profile_picture ? (
                ...
                <div 
                  className={`w-full h-full flex items-center justify-center text-white text-xl font-black bg-gradient-to-br ${roleCfg.gradient}`}
                >
                  {userData.full_name?.charAt(0)?.toUpperCase()}
                </div>
              )}
            ...
            {/* Role icon badge */}
            <div 
              className={`absolute -bottom-1.5 -right-1.5 w-5 h-5 rounded-lg bg-gradient-to-br ${roleCfg.gradient} flex items-center justify-center shadow-md`}
            >
          ...
          <div className="flex-1 min-w-0 pt-0.5">
            ...
            <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
              <span 
                className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-bold text-white bg-gradient-to-r ${roleCfg.gradient}`}
              >
        ...
        {isPending && isAdmin && (
          <div className="flex gap-2 mt-4 pt-3 border-t border-amber-100">
            <Button 
              ...
              disabled={approving === userData.id} 
              onClick={() => onApprove(userData)}
              className="flex-1 h-8 rounded-2xl text-white font-bold text-xs gap-1 shadow-sm hover:shadow-md transition-all"
              style={{ background: COLORS.emeraldGreen }}
            >
              <UserCheck className="h-3.5 w-3.5" />
              {approving === userData.id ? 'Processing…' : 'Approve'}
...
const PermToggleRow = ({ 
  ...
  return (
    <div 
      className={`flex items-center justify-between px-4 py-3 rounded-2xl transition-all duration-200 border ${
        isOn
...
const ModuleAccessCard = ({ 
  ...
  return (
    <div
      ...
      className={`relative flex items-center gap-4 p-4 rounded-3xl border-2 cursor-pointer transition-all duration-300 select-none group hover:-translate-y-1 hover:scale-[1.01] ${
        isEnabled
          ? 'shadow-md'
    ...
    >
      ...
      <div 
        className={`w-6 h-6 rounded-xl flex items-center justify-center flex-shrink-0 font-black text-xs transition-all duration-200 ${
...
const PermissionMatrixSummary = ({ permissions }) => {
  ...
  return (
    <div className="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-700/50 rounded-3xl border border-slate-200 dark:border-slate-600 shadow-sm transition-all duration-300">
  ...
  return (
    ...
    >
      ...
      <motion.div variants={slideIn} className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 font-medium">
                Team administration &amp; access control
              </p>
            </div>
          </div>
        </div>
        {isAdmin && (
          <Button
            className="rounded-2xl shadow-sm hover:shadow-md transition-all font-bold h-11 px-6 text-white hover:scale-105"
            style={{ background: GRADIENT }}
            onClick={() => {
        ...
        {stats.map((s, i) => {
          ...
          return (
            <motion.div 
              key={s.label} 
              variants={itemVariants}
              className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 p-4 flex items-center gap-3 shadow-md hover:shadow-xl transition-all duration-300 hover:-translate-y-1 hover:scale-[1.01]"
            >
              <div 
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto rounded-3xl p-0 border-none shadow-2xl">
          ...
          <div className="p-6 space-y-8 bg-white dark:bg-slate-900">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Input 
                  ...
                  className="rounded-xl h-11 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700" 
                />
              </div>
              <div className="space-y-1.5">
                <Label className="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider">
            ...
            {/* Shift schedule */}
            <div className="rounded-3xl border border-blue-100 dark:border-blue-900/40 bg-blue-50/50 dark:bg-blue-900/10 p-5 shadow-sm hover:shadow-md transition-all duration-300">
                ...
                {[
                  ...
                  <div key={f.name} className="space-y-1.5">
                    ...
                    <Input 
                      ...
                      onChange={handleInput}
                      className="rounded-xl h-10 bg-white dark:bg-slate-800 border-blue-200 dark:border-slate-600" 
                    />
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <Select value={formData.role} onValueChange={handleRoleChange}>
                    <SelectTrigger className="rounded-xl h-11 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  {roleChanged && selectedUser && (
                    <div className="flex items-center gap-3 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl mt-2">
                      ...
                      <button 
                        ...
                        className="text-[11px] font-bold px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-white rounded-lg whitespace-nowrap"
                      >
                        Reset
                      </button>
                      <button 
          ...
          <div className="sticky bottom-0 px-6 py-4 bg-slate-50 dark:bg-slate-800/80 border-t dark:border-slate-700 flex justify-end gap-3 rounded-b-3xl">
            <Button 
              ...
              className="rounded-2xl shadow-sm hover:shadow-md transition-all h-11 px-6"
            >
              Discard
            </Button>
            <Button 
              ...
              disabled={loading}
              className="rounded-2xl shadow-sm hover:shadow-md transition-all h-11 px-8 font-bold text-white hover:opacity-90"
      ...
      <AnimatePresence>
        {isAdmin && pendingUsers.length > 0 && activeTab !== 'pending' && (
          <motion.div
            ...
            className="flex items-center gap-4 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-3xl shadow-sm hover:shadow-md transition-all duration-300"
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-amber-500 mt-0.5">
                These users cannot log in until approved.
              </p>
            </div>
            <Button 
              size="sm" 
              onClick={() => setActiveTab('pending')}
              className="rounded-2xl shadow-sm hover:shadow-md transition-all bg-amber-500 hover:bg-amber-600 text-white font-bold text-xs h-8"
            >
              Review Now 
      ...
      <div className="flex flex-col lg:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="pl-11 h-11 rounded-3xl border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-sm transition-all duration-300 hover:shadow-md focus:shadow-md" 
          />
        </div>
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-1 rounded-3xl shadow-sm flex flex-wrap gap-0.5 self-start transition-all duration-300">
          ...
          {isAdmin && (
            <button
              onClick={() => setActiveTab('pending')}
              className={`relative px-4 py-2 rounded-2xl text-xs font-bold transition-all flex items-center gap-1.5 ${
          ...
          {isAdmin && rejectedUsers.length > 0 && (
            <button
              ...
              className={`px-4 py-2 rounded-2xl text-xs font-bold transition-all flex items-center gap-1.5 ${
                activeTab === 'rejected' ? 'bg-red-500 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:text-slate-400'
              }`}
            >
      ...
      {/* ══════════════════════════════════════════════════════════════════
          PERMISSIONS DIALOG — Fixed and Upscaled
      ══════════════════════════════════════════════════════════════════ */}
        ...
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto rounded-3xl p-0 border-none shadow-2xl">
          ...
          <div className="sticky top-0 z-10 rounded-t-3xl overflow-hidden">
            ...
            <div className="p-6 bg-white dark:bg-slate-900 border-b dark:border-slate-700">
              ...
              {/* ✅ REQUIRED FIX: Added DialogTitle */}
              <DialogTitle className="text-xl font-black flex items-center gap-3" style={{ color: COLORS.deepBlue }}>
                <Shield className="h-5 w-5" />
                {selectedUserForPerms?.full_name} — Permissions
              </DialogTitle>

              {/* ✅ REQUIRED FIX: Added DialogDescription */}
              <DialogDescription className="text-xs text-slate-400 mt-1">
                Configure module access, operational limits, and view controls for this user. Changes apply instantly after saving.
              </DialogDescription>
          ...
          <div className="p-6 space-y-8 bg-white dark:bg-slate-900">
            ...
            <Tabs value={activePermTab} onValueChange={setActivePermTab}>
              <TabsList className="grid grid-cols-3 md:grid-cols-6 gap-2 bg-slate-100 dark:bg-slate-800 p-1 rounded-3xl shadow-sm mb-6 h-auto">
                {permTabs.map(tab => {
                  const Icon = tab.icon;
                  return (
                    <TabsTrigger
                      key={tab.id}
                      value={tab.id}
                      className="rounded-2xl py-2 text-xs font-bold flex items-center gap-1.5 justify-center transition-all data-[state=active]:shadow-md data-[state=active]:bg-white dark:data-[state=active]:bg-slate-700"
                    >
                      <Icon className="h-3.5 w-3.5" />
                      {tab.label}
                    </TabsTrigger>
              ...
              {/* TAB CONTENTS: Implemented cleanly with Framer Motion for smooth transitions */}
              ...
              <TabsContent value="modules" className="mt-0 outline-none">
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
                  <SectionHeader 
                    icon={Zap} 
                    title="Specialised Module Access" 
                    color={COLORS.indigo}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <ModuleAccessCard 
                      ...
                      desc="Manage leads pipeline end-to-end"
                    ...
                    <ModuleAccessCard 
                      icon={Receipt} 
                      title="Quotations"
                      desc="Create and export quotations"
                    ...
                    <ModuleAccessCard 
                      ...
                      desc="Access and manage credentials"
                  ...
                  {/* Vault state feedback notifications */}
                  {permissions.can_edit_passwords && !permissions.can_view_passwords && (
                    <div className="flex items-center gap-2 mt-3 px-4 py-3 rounded-2xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-xs text-amber-700 dark:text-amber-400 font-medium shadow-sm transition-all duration-300">
                      <AlertCircle className="h-4 w-4 flex-shrink-0" />
                      Edit vault requires View vault to also be enabled.
                    </div>
                  )}
                  {permissions.can_view_passwords && permissions.can_edit_passwords && (
                    <div className="flex items-center gap-2 mt-3 px-4 py-3 rounded-2xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-xs text-emerald-700 dark:text-emerald-400 font-medium shadow-sm transition-all duration-300">
                      <CheckCircle className="h-4 w-4 flex-shrink-0" />
                      Full password vault access — user can reveal and manage credentials.
                    </div>
                  )}

                  {/* Password dept access */}
                  {permissions.can_view_passwords && (
                    <div className="rounded-3xl border border-teal-200 dark:border-teal-800 p-6 bg-teal-50/40 dark:bg-teal-900/10 shadow-sm transition-all duration-300 hover:shadow-md mt-6">
                      <SectionHeader 
                        icon={KeyRound} 
                        title="Password Vault — Department Access" 
                        color={COLORS.teal} 
                      />
                      ...
                      <p className="text-[10px] text-slate-400 mt-4 font-medium px-2 border-l-2 border-teal-300">
                        Current Scope: {(permissions.view_password_departments || []).length === 0
              ...
              <TabsContent value="view" className="mt-0 outline-none">
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                  <SectionHeader 
                    icon={Eye} 
                    title="Global Visibility Permissions" 
                    color="#3B82F6"
              ...
              <TabsContent value="edit" className="mt-0 outline-none">
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                  <SectionHeader 
                    icon={Pencil} 
                    title="Edit & Modification Permissions" 
                    color="#F59E0B"
              ...
              <TabsContent value="cross" className="mt-0 outline-none">
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                  <SectionHeader 
                    icon={UsersIcon} 
                    title="Cross-User Data Visibility" 
                    color={COLORS.emeraldGreen} 
                    desc="Choose which team members' data this user can see. Click a name to toggle access."
                  />
                  ...
                  {[
                    ...
                    return (
                      <div 
                        key={section.key} 
                        className="rounded-3xl border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm hover:shadow-md transition-all duration-300"
                      >
                        <div className="flex items-center gap-2.5 px-5 py-4 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                          <div 
                          ...
                          <p className="text-sm font-bold text-slate-700 dark:text-slate-200 flex-1">
                          ...
                          {users.filter(u => u.id !== selectedUserForPerms?.id).map(u => {
                            ...
                            return (
                              <button 
                                ...
                                className={`px-4 py-2 rounded-2xl text-xs font-bold border-2 transition-all shadow-sm hover:shadow-md hover:-translate-y-0.5 ${
                                  sel
                                    ? 'text-white border-transparent'
              ...
              <TabsContent value="clients" className="mt-0 outline-none">
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                  <SectionHeader 
                    ...
                    color={COLORS.teal} 
                    desc="Select specific clients that this user has permission to manage."
                  />
                  <div className="flex items-center justify-between">
                    ...
                    {(permissions.assigned_clients || []).length > 0 && (
                      <button 
                        ...
                        className="text-xs font-bold text-red-500 hover:text-red-700 px-3 py-1.5 rounded-xl hover:bg-red-50 transition-colors shadow-sm"
                      >
                        Clear all
                      </button>
                    )}
                  ...
                  <div className="relative">
                    ...
                    <Input 
                      ...
                      className="pl-12 h-12 rounded-2xl shadow-sm hover:shadow-md transition-all dark:bg-slate-800 dark:border-slate-700" 
                    />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                      ...
                      .map(client => {
                        ...
                        return (
                          <button 
                            ...
                            className={`flex items-center gap-4 p-4 rounded-2xl border-2 text-left transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md ${
                              assigned
                          ...
                          >
                            ...
                            >
                              {assigned ? <CheckCircle className="h-4 w-4" /> : <Hash className="h-4 w-4" />}
                            </div>
                            <span 
                    ...
                    {clients.filter(c => c.company_name.toLowerCase().includes(clientSearch.toLowerCase())).length === 0 && (
                      <div className="col-span-2 py-12 text-center text-sm font-medium text-slate-400">
                        No clients match your search parameters.
                      </div>
                    )}
                  </div>
                </motion.div>
          ...
          {/* 🔥 Footer */}
          <DialogFooter className="sticky bottom-0 px-6 py-5 bg-slate-50 dark:bg-slate-800 border-t dark:border-slate-700 flex flex-col sm:flex-row items-center justify-between rounded-b-3xl gap-4">
            
            <Button
              variant="outline"
              onClick={() => resetPermissionsToRole(selectedUserForPerms?.role)}
              className="rounded-2xl font-bold text-xs shadow-sm hover:shadow-md transition-all h-11 px-5 border-slate-200 dark:border-slate-600"
            >
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              <Button 
                ...
                className="rounded-2xl shadow-sm hover:shadow-md transition-all h-11 px-6 font-semibold"
              >
                Discard
              </Button>
              ...
              <Button
                ...
                disabled={loading}
                className="rounded-2xl px-8 h-11 text-white font-bold shadow-md hover:shadow-lg transition-all hover:-translate-y-0.5"
                style={{ background: GRAD_GREEN }}
              >
