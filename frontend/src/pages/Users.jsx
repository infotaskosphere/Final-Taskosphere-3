...
const PendingUserCard = ({ userData, onApprove, onReject, approving }) => (
  <motion.div variants={itemVariants} layout
    className="relative bg-white dark:bg-slate-800 rounded-2xl overflow-hidden border border-amber-200 shadow-sm hover:shadow-md transition-all">
    {/* Top stripe */}
    ...
    <div className="p-5">
      ...
      <div className="flex gap-2 pt-3 border-t border-amber-100">
        <Button size="sm" disabled={approving === userData.id} onClick={() => onApprove(userData)}
          className="flex-1 h-8 rounded-xl text-white font-bold text-xs gap-1.5 shadow-sm"
        ...
        <Button size="sm" disabled={approving === userData.id} onClick={() => onReject(userData)}
          variant="outline"
          className="flex-1 h-8 rounded-xl border-red-200 text-red-600 hover:bg-red-50 font-bold text-xs gap-1.5">
...
const UserCard = ({ userData, onEdit, onDelete, onPermissions, onApprove, onReject,
  ...
  return (
    <motion.div variants={itemVariants} layout
      className={`group relative bg-white dark:bg-slate-800 rounded-2xl overflow-hidden border transition-all duration-300 ${
        isPending
          ? 'border-amber-300 shadow-amber-100/50 shadow-md'
          : hovered
          ? 'border-blue-200 shadow-xl shadow-blue-100/30 -translate-y-1'
    ...
    >
          ...
          <motion.div
            ...
            {canManagePermissions && userData.role !== 'admin' && !isPending && (
              <button onClick={() => onPermissions(userData)}
                className="w-8 h-8 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-600 flex items-center justify-center shadow-sm border border-emerald-200 transition-colors"
            ...
            {(isAdmin || (canEditUsers && !isPending)) && (
              <button onClick={() => onEdit(userData)}
                className="w-8 h-8 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-600 flex items-center justify-center shadow-sm border border-blue-200 transition-colors"
                title="Edit">
            ...
            {(isAdmin || canEditUsers) && userData.id !== currentUserId && (
              <button onClick={() => onDelete(userData.id)}
                className="w-8 h-8 rounded-xl bg-red-50 hover:bg-red-100 text-red-600 flex items-center justify-center shadow-sm border border-red-200 transition-colors"
                title="Delete">
      ...
      <div className="p-5">
        ...
        <div className="flex items-start gap-3.5 mb-4">
          <div className="relative flex-shrink-0">
            <div className="w-13 h-13 w-[52px] h-[52px] rounded-2xl overflow-hidden shadow-sm">
              {userData.profile_picture
                ? <img src={userData.profile_picture} alt={userData.full_name} className="w-full h-full object-cover" />
        ...
        {isPending && isAdmin && (
          <div className="flex gap-2 mt-4 pt-3 border-t border-amber-100">
            <Button size="sm" disabled={approving === userData.id} onClick={() => onApprove(userData)}
              className="flex-1 h-8 rounded-xl text-white font-bold text-xs gap-1 shadow"
              style={{ background: COLORS.emeraldGreen }}>
              <UserCheck className="h-3.5 w-3.5" />
              {approving === userData.id ? 'Processing…' : 'Approve'}
            </Button>
...
const PermToggleRow = ({ permKey, label, desc, icon: Icon, permissions, setPermissions }) => {
  ...
  return (
    <div className={`flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-200 border ${
      isOn
...
const ModuleAccessCard = ({ icon: Icon, title, desc, permKey, permissions, setPermissions, accentColor, badge }) => {
  ...
  return (
    <div
      ...
      className={`relative flex items-center gap-4 p-4 rounded-2xl border-2 cursor-pointer transition-all duration-200 select-none group ${
        isEnabled
          ? 'shadow-sm'
...
export default function Users() {
  ...
  return (
    <motion.div
      className={`space-y-6 p-4 md:p-8 min-h-screen ${isDark ? 'bg-[#0b1120]' : 'bg-slate-50/50'}`}
        ...
        {stats.map((s, i) => {
          ...
          return (
            <motion.div key={s.label} variants={itemVariants}
              className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 p-4 flex items-center gap-3 shadow-sm hover:shadow-md transition-shadow">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto rounded-3xl p-0 border-none shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Input name="full_name" value={formData.full_name} onChange={handleInput}
                  placeholder="e.g. Manthan Desai" className="rounded-xl h-11 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700" />
              </div>
              <div className="space-y-1.5">
                ...
                <Input type="email" name="email" value={formData.email} onChange={handleInput}
                  placeholder="name@firm.com" className="rounded-xl h-11 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Input name="phone" value={formData.phone} onChange={handleInput}
                  placeholder="+91 00000 00000" className="rounded-xl h-11 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700" />
              </div>
              <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input type="number" name="telegram_id" value={formData.telegram_id} onChange={handleInput}
                  placeholder="Numeric Telegram ID" className="rounded-xl h-11 bg-slate-50 dark:bg-slate-800 border-slate-200 dark:border-slate-700" />
              </div>
            </div>
          ...
          <div className="sticky bottom-0 px-6 py-4 bg-slate-50 dark:bg-slate-800/80 border-t dark:border-slate-700 flex justify-end gap-3 rounded-b-3xl">
            <Button variant="ghost" onClick={() => setDialogOpen(false)} className="rounded-xl h-11 px-6">Discard</Button>
            <Button onClick={handleSubmit} disabled={loading}
              className="rounded-xl h-11 px-8 font-bold shadow-lg text-white hover:opacity-90"
      ...
      <AnimatePresence>
        {isAdmin && pendingUsers.length > 0 && activeTab !== 'pending' && (
          <motion.div
            ...
            className="flex items-center gap-4 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl shadow-sm">
            ...
            <Button size="sm" onClick={() => setActiveTab('pending')}
              className="rounded-xl bg-amber-500 hover:bg-amber-600 text-white font-bold text-xs h-8 shadow-sm">
      ...
      {/* ══════════════════════════════════════════════════════════════════
          PERMISSIONS DIALOG — Full redesign with tab-based navigation
      ══════════════════════════════════════════════════════════════════ */}
      <Dialog open={permDialogOpen} onOpenChange={setPermDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[92vh] overflow-hidden rounded-3xl p-0 border-none shadow-2xl flex flex-col">

          {/* Dialog header */}
          <div className="flex-shrink-0">
            <div className="h-1.5 w-full" style={{ background: 'linear-gradient(90deg, #10b981, #1F6FB2, #7C3AED)' }} />
            <div className="p-5 bg-white dark:bg-slate-900 border-b dark:border-slate-700">
              <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      ...
                      {['staff','manager','admin'].map(r => (
                        <button key={r} type="button"
                          ...
                          className={`text-[9px] px-2 py-1 rounded-lg font-bold border transition-all hover:scale-105 ${
                ...
                <Button variant="ghost" size="sm" className="rounded-xl text-slate-400 hover:text-slate-600"
                  onClick={() => setPermDialogOpen(false)}>✕</Button>
              </div>
            </div>
          ...
          {/* Scrollable content */}
          <div className="flex-1 overflow-y-auto p-5 bg-white dark:bg-slate-900 space-y-4">
              ...
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
                ...
                <div>
                  ...
                  {permissions.can_edit_passwords && !permissions.can_view_passwords && (
                    <div className="flex items-center gap-2 mt-3 px-4 py-3 rounded-xl bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-xs text-amber-700 dark:text-amber-400 font-medium">
                      <AlertCircle className="h-4 w-4 flex-shrink-0" />
                      Edit vault requires View vault to also be enabled.
                    </div>
                  )}
                  {permissions.can_view_passwords && permissions.can_edit_passwords && (
                    <div className="flex items-center gap-2 mt-3 px-4 py-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 text-xs text-emerald-700 dark:text-emerald-400 font-medium">
                ...
                {permissions.can_view_passwords && (
                  <div className="rounded-2xl border-2 border-teal-200 dark:border-teal-800 p-5 bg-teal-50/40 dark:bg-teal-900/10">
            ...
            {activePermTab === 'cross' && (
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
                ...
                {[
                  ...
                  return (
                    <div key={section.key} className="rounded-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
              ...
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                ...
                <div className="relative">
                  ...
                  <Input placeholder="Search companies…" value={clientSearch}
                    ...
                    className="pl-10 h-10 rounded-xl dark:bg-slate-800 dark:border-slate-700" />
                </div>
          ...
          {/* Sticky footer */}
          <div className="flex-shrink-0 px-5 py-4 bg-slate-50 dark:bg-slate-800 border-t dark:border-slate-700 flex items-center justify-between gap-3 rounded-b-3xl">
            <div className="flex items-center gap-2 text-[11px] text-slate-400">
              <SlidersHorizontal className="h-3.5 w-3.5" />
              {Object.entries(permissions).filter(([k, v]) => k.startsWith('can_') && v === true).length} permissions active
            </div>
            <div className="flex gap-3">
              <Button variant="ghost" className="rounded-xl h-10 px-5 text-sm" onClick={() => setPermDialogOpen(false)}>Discard</Button>
              <Button onClick={handleSavePermissions} disabled={loading}
                className="rounded-xl h-10 px-7 font-bold shadow-lg text-white text-sm"
                style={{ background: GRAD_GREEN }}>
