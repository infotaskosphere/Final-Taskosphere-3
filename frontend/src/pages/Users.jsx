...
const GLOBAL_PERMS = [
  {
    ...
    label: 'Universal Task Access',
    desc: 'See tasks assigned to any user or department',
    icon: Layers,
  },
  {
    key: 'can_view_all_clients',
    label: 'Master Client List',
    desc: 'View all company legal entities',
    icon: Briefcase,
  },
  {
    key: 'can_view_all_dsc',
    label: 'DSC Vault Access',
    desc: 'View all Digital Signature Certificates',
    icon: Fingerprint,
  },
  {
    key: 'can_view_documents',
    label: 'Document Library',
    desc: 'Access physical document register',
    icon: FileText,
  },
  {
    key: 'can_view_all_duedates',
    label: 'Compliance Roadmap',
    desc: 'View all upcoming statutory due dates',
    icon: Calendar,
  },
  {
    key: 'can_view_reports',
    label: 'Analytics Dashboard',
    desc: 'View performance and system-wide reports',
    icon: BarChart2,
  },
  {
    key: 'can_view_todo_dashboard',
    label: 'Todo Dashboard',
    desc: 'Access global team todo overview',
    icon: CheckCircle,
  },
  {
    key: 'can_view_audit_logs',
    label: 'System Audit Trail',
    desc: 'View activity logs and record histories',
    icon: Activity,
  },
  {
    key: 'can_view_all_leads',
    label: 'Leads Pipeline',
    desc: 'View the global leads dashboard',
    icon: Target,
  },
  {
    key: 'can_view_user_page',
    label: 'User Directory',
    desc: 'View team members directory',
    icon: UsersIcon,
  },
  {
    key: 'can_view_selected_users_reports',
    label: 'Team Reports Access',
    desc: 'View reports for selected users',
    icon: Eye,
  },
  {
    key: 'can_view_staff_rankings',
    label: 'Staff Rankings',
    desc: 'View performance leaderboard',
    icon: Star,
  },
  {
    key: 'can_view_own_data',
    label: 'View Own Data',
    desc: 'Access own attendance, tasks and reports',
    icon: UserIcon,
  },
  {
    key: 'can_create_quotations',
    label: 'Quotations Module',
...
const OPS_PERMS = [
  {
    ...
    desc: 'Assign tasks to other staff members',
    icon: ArrowUpRight,
  },
  {
    key: 'can_assign_clients',
    label: 'Client Assignment',
    desc: 'Assign and reassign staff to clients',
    icon: Briefcase,
  },
  {
  ...
  },
  {
    ...
    desc: 'Review punch timings and late reports',
    icon: Clock,
  },
  {
    key: 'can_view_staff_activity',
    label: 'Staff Monitoring',
    desc: 'View app usage and screen activity',
    icon: Activity,
  },
  {
    key: 'can_send_reminders',
    label: 'Automated Reminders',
    desc: 'Trigger email/notification reminders',
    icon: Bell,
  },
  {
    key: 'can_download_reports',
    label: 'Export Data',
    desc: 'Download CSV/PDF versions of reports',
    icon: Download,
  },
  {
    key: 'can_manage_settings',
    label: 'System Settings',
    desc: 'Modify global system configuration',
    icon: Settings,
  },
  {
    key: 'can_delete_data',
    label: 'Delete Records',
    desc: 'Permanently delete data entries',
    icon: Trash2,
  },
  {
    key: 'can_delete_tasks',
    label: 'Delete Tasks',
    desc: 'Delete any task regardless of ownership',
    icon: XCircle,
  },
  {
    key: 'can_connect_email',
    label: 'Connect Email Accounts',
    desc: 'Link personal email via IMAP integration',
    icon: Inbox,
  },
] as const;
...
const EDIT_PERMS = [
  {
    key: 'can_edit_tasks',
    label: 'Modify Tasks',
    desc: 'Update and delete task definitions',
    icon: Pencil,
  },
  {
    key: 'can_edit_clients',
    label: 'Modify Clients',
    desc: 'Update client master data records',
    icon: Edit,
  },
  {
    key: 'can_edit_dsc',
    label: 'Modify DSC',
    desc: 'Update certificate details and metadata',
    icon: Fingerprint,
  },
  {
    key: 'can_edit_documents',
    label: 'Modify Documents',
    desc: 'Change document records',
    icon: FileText,
  },
  {
    key: 'can_edit_due_dates',
    label: 'Modify Due Dates',
    desc: 'Edit statutory compliance timelines',
    icon: Calendar,
  },
  {
    key: 'can_edit_users',
    label: 'Modify Users',
    desc: 'Update user profiles and settings',
    icon: UserIcon,
  },
] as const;
...
const DeptPill: React.FC<{ dept: string; size?: 'sm' | 'md' }> = ({ dept }) => {
...
const StatusBadge: React.FC<{ status?: string; isActive?: boolean }> = ({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-xs font-semibold ${cfg.cls}`}
    >
      <span
        className={`w-2 h-2 rounded-full ${cfg.dot} ${
          resolved === 'active' ? 'animate-pulse' : ''
        }`}
      />
      {cfg.label}
    </span>
...
const UserCard: React.FC<UserCardProps> = ({
  ...
  const roleCfg =
    ROLE_CONFIG[userData.role?.toLowerCase() as keyof typeof ROLE_CONFIG] ||
    ROLE_CONFIG.staff;
  const RoleIcon = roleCfg.icon;

  const permCount = useMemo(() => {
    return userData.permissions
      ? Object.entries(userData.permissions).filter(
          ([k, v]) => k.startsWith('can_') && v === true
        ).length
      : 0;
      ...
      <div className="p-6">
        <div className="flex items-start gap-5">
          <div className="relative flex-shrink-0">
            <div className="w-[68px] h-[68px] rounded-2xl overflow-hidden shadow-md ring-1 ring-slate-100 dark:ring-slate-700">
              {userData.profile_picture ? (
                ...
                <div
                  className={`w-full h-full flex items-center justify-center text-white text-3xl font-black bg-gradient-to-br ${roleCfg.gradient}`}
                >
                  {userData.full_name?.charAt(0)?.toUpperCase()}
                </div>
              )}
            ...
            <div
              className={`absolute -bottom-1.5 -right-1.5 w-7 h-7 rounded-2xl bg-gradient-to-br ${roleCfg.gradient} flex items-center justify-center ring-2 ring-white dark:ring-slate-900 shadow`}
            >
          ...
          <div className="flex-1 min-w-0 pt-1">
            <h3 className="font-semibold text-lg tracking-tight text-slate-900 dark:text-white truncate">
              {userData.full_name}
            </h3>
            <div className="flex items-center gap-3 mt-3">
              <div
                className={`inline-flex items-center gap-1.5 px-4 py-1 rounded-2xl text-xs font-bold text-white bg-gradient-to-r ${roleCfg.gradient}`}
              >
...
const PermToggleRow: React.FC<PermToggleRowProps> = ({
  permKey,
  label,
  desc,
  icon: Icon,
...
const ModuleAccessCard: React.FC<ModuleAccessCardProps> = ({
  ...
  return (
    <div
      onClick={() =>
        setPermissions((p) => ({ ...p, [permKey]: !p[permKey] }))
      }
...
const SectionHeader: React.FC<SectionHeaderProps> = ({
  ...
  <div className="flex items-center gap-4 mb-6">
    ...
    <div>
      <p className="font-bold text-xl tracking-tight text-slate-900 dark:text-white">
        {title}
      </p>
    </div>
    {count !== undefined && (
...
const PermissionMatrixSummary: React.FC<PermissionMatrixSummaryProps> = ({
  permissions,
}) => {
...
export default function Users() {
  ...
  const [activePermTab, setActivePermTab] = useState<
    'modules' | 'view' | 'ops' | 'edit' | 'cross' | 'clients'
  >('modules');
  ...
  const handlePhoto = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () =>
      setFormData((p) => ({ ...p, profile_picture: reader.result as string }));
  ...
  const handleEdit = useCallback((userData: UserData) => {
    ...
    setFormData({
      ...
      telegram_id:
        userData.telegram_id != null ? String(userData.telegram_id) : '',
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        typeof detail === 'string' ? detail : 'Failed to save user'
      );
    } finally {
      setLoading(false);
    }
  ...
  const resetPermissionsToRole = useCallback((role: string) => {
    setPermissions({
      ...(DEFAULT_ROLE_PERMISSIONS[
        role as keyof typeof DEFAULT_ROLE_PERMISSIONS
      ] || EMPTY_PERMISSIONS),
    });
  ...
  const filteredUsers = useMemo(() => {
    return users.filter((u) => {
      ...
      if (activeTab === 'pending')
        return match && u.status === 'pending_approval';
      if (activeTab === 'rejected')
        return match && u.status === 'rejected';
      ...
      <div className="flex items-center justify-center min-h-screen bg-slate-50 dark:bg-slate-950 p-8">
        ...
        >
          ...
          <p className="text-slate-600 dark:text-slate-400">
            You need the{' '}
            <span className="font-semibold">View User Directory</span>{' '}
  ...
  return (
    <motion.div
      className={`space-y-10 p-6 md:p-10 min-h-screen ${
        isDark ? 'bg-[#0a0f1c]' : 'bg-slate-50'
      }`}
      initial="hidden"
      animate="visible"
    ...
    >
      ...
      >
        <div className="flex items-center gap-5">
          <div
            className="w-14 h-14 rounded-3xl flex items-center justify-center shadow-xl"
        ...
        <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto rounded-3xl p-0 border-0 shadow-2xl">
          ...
          <div className="p-8 space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <Label className="text-xs font-semibold tracking-widest text-slate-500 mb-2 block">
                  FULL NAME
                </Label>
                <Input
                  name="full_name"
            ...
            {isAdmin && (
              <>
                ...
                {/* Departments */}
                <div>
                  <Label className="text-xs font-semibold tracking-widest text-slate-500 mb-4 block">
                    ASSIGNED DEPARTMENTS
                  </Label>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                    {DEPARTMENTS.map((dept) => {
                      const active = formData.departments.includes(
                        dept.value
                      );
                      return (
                        <button
                          ...
                          className={`h-12 rounded-2xl text-sm font-semibold border-2 transition-all ${
                            active
                              ? 'text-white border-transparent shadow'
                              : 'border-slate-200 dark:border-slate-700 hover:border-slate-300'
                          }`}
                          style={{
                            background: active ? dept.color : undefined,
                          }}
                        >
                          {dept.label}
                        </button>
          ...
          <DialogFooter className="px-8 py-6 border-t dark:border-slate-700 bg-slate-50 dark:bg-slate-900 rounded-b-3xl flex justify-end gap-4">
            ...
            <Button
              ...
              disabled={loading}
              className="h-12 px-10 rounded-2xl font-semibold text-base"
      ...
      >
        {[
          ...
          <button
            ...
            style={
              activeTab === tab.id ? { background: GRADIENT } : {}
            }
          >
            {tab.label}
          </button>
              ...
              {permTabs.map((tab) => {
                ...
                return (
                  <button
                    key={tab.id}
                    onClick={() =>
                      setActivePermTab(tab.id as any)
                    }
                    className={`px-5 py-2.5 rounded-2xl font-semibold text-sm transition-all whitespace-nowrap flex items-center gap-2 ${
              ...
              {activePermTab === 'ops' && (
                <div>
                  <SectionHeader
                    ...
                    color="#8B5CF6"
                    count={
                      OPS_PERMS.filter((p) => permissions[p.key])
                        .length
                    }
                  />
                  <div className="space-y-3">
              ...
              {activePermTab === 'edit' && (
                <div>
                  <SectionHeader
                    ...
                    color="#F59E0B"
                    count={
                      EDIT_PERMS.filter((p) => permissions[p.key])
                        .length
                    }
                  />
                  <div className="space-y-3">
              ...
              {activePermTab === 'cross' && (
                <div className="space-y-8">
                  ...
                  <p className="text-slate-500 text-sm -mt-3">
                    Select team members whose data this user can view
                  </p>
                  {[
                    {
                      key: 'view_other_tasks',
                      label: 'Tasks',
                      icon: Layers,
                      color: '#3B82F6',
                    },
                    {
                      key: 'view_other_attendance',
                      label: 'Attendance',
                      icon: Clock,
                      color: '#8B5CF6',
                    },
                    ...
                    {
                      key: 'view_other_todos',
                      label: 'Todos',
                      icon: CheckCircle,
                      color: '#10B981',
                    },
                    {
                      key: 'view_other_activity',
                      label: 'Activity',
                      icon: Activity,
                      color: '#EF4444',
                    },
                    ...
                    const selectedCount = (
                      permissions[
                        section.key as keyof typeof permissions
                      ] || []
                    ).length;
                    ...
                    return (
                      <div
                        key={section.key}
                        className="border rounded-3xl overflow-hidden bg-white dark:bg-slate-900"
                      >
                                  ...
                                  onClick={() =>
                                    setPermissions((prev) => ({
                                      ...
                                      [section.key]: isSelected
                                        ? (
                                            prev[
                                              section.key as keyof typeof prev
                                            ] || []
                                          ).filter(
                                            (id: string) =>
                                              id !== u.id
                            ...
                            onClick={() =>
                              setPermissions((prev) => ({
                                ...
                                assigned_clients: isAssigned
                                  ? (prev.assigned_clients || []).filter(
                                      (id: string) =>
                                        id !== client.id
                                    )
                                  : [
                                      ...(prev.assigned_clients || []),
                                      client.id,
                                    ],
                              }))
                            }
