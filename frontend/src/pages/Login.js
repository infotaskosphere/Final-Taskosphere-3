...
function TaskStrip({ task, isToMe, assignedName, onUpdateStatus, navigate }) {
 ...
 return (
  <motion.div
   ...
   className={`relative flex flex-col p-5 rounded-3xl border bg-white transition-all cursor-pointer group ${getPriorityStripeClass(task.priority)} ${isCompleted ? 'opacity-80 bg-green-50/40 border-green-200' : 'hover:shadow-2xl hover:border-blue-400 hover:ring-1 hover:ring-blue-200/60'}`}
   onClick={() => navigate(`/tasks?filter=assigned-to-me&taskId=${task.id}`)}
  >
   {/* Title + Action Buttons */}
   <div className="flex items-start justify-between gap-3">
    <div className="flex-1 min-w-0">
     <p className={`font-medium truncate leading-tight transition ${isCompleted ? 'line-through text-slate-500' : 'text-slate-900'}`}>
      {task.title || 'Untitled Task'} {task.client_name ? ` – ${task.client_name}` : ''}
     </p>
    </div>
    ...
    {isToMe && (
     <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
      ...
      <motion.button
       ...
       className={`w-28 text-center py-1 text-xs font-medium rounded-full transition ${isInProgress ? 'bg-blue-600 text-white shadow' : 'bg-white border border-blue-400 text-blue-700 hover:bg-blue-50'} disabled:opacity-50`}
      >
      ...
      <motion.button
       ...
       disabled={isCompleted}
       className={`w-28 text-center py-1 text-xs font-medium rounded-full transition ${isCompleted ? 'bg-green-600 text-white' : 'bg-green-100 text-green-800 border border-green-300 hover:bg-green-200'}`}
      >
   ...
   <div className="mt-2 text-xs text-slate-500 flex flex-wrap gap-x-3 gap-y-1">
    <span>
     {isToMe ? 'Assigned by: ' : 'Assigned to: '} <span className="font-medium text-slate-700">{assignedName || 'Unknown'}</span>
    </span>
    <span>
...
export default function Dashboard() {
 ...
 const todos = useMemo(() => todosRaw.map(todo => ({ ...todo, completed: todo.status === "completed", })), [todosRaw]);
 ...
 const updateAssignedTaskStatus = (taskId, newStatus) => {
  updateTaskMutation.mutate( { id: taskId, data: { status: newStatus, updated_at: new Date().toISOString(), }, }, { onSuccess: () => { toast.success(newStatus === 'completed' ? 'Task completed!' : 'Task in progress!'); queryClient.invalidateQueries({ queryKey: ['tasks'] }); queryClient.invalidateQueries({ queryKey: ['dashboardStats'] }); }, onError: (err) => { console.error("Update Error:", err); toast.error(err.response?.data?.detail || 'Failed to update task'); }, } );
 };
 ...
 const nextDeadline = upcomingDueDates.length > 0 ? upcomingDueDates.reduce((prev, curr) => prev.days_remaining < curr.days_remaining ? prev : curr ) : null;
 const isAdmin = user?.role === 'admin';
 const showTaskSection = isAdmin || tasksAssignedToMe.length > 0 || tasksAssignedByMe.length > 0;
 const isOverdue = (dueDate) => dueDate && new Date(dueDate) < new Date();
 const getStatusStyle = (status) => {
  const styles = { completed: { bg: 'bg-emerald-100', text: 'text-emerald-700', dot: 'bg-emerald-500' }, in_progress: { bg: 'bg-blue-100', text: 'text-blue-700', dot: 'bg-blue-500' }, pending: { bg: 'bg-slate-100', text: 'text-slate-700', dot: 'bg-slate-400' }, };
  return styles[status] || styles.pending;
 };
 const getPriorityStyle = (priority) => {
  const styles = { high: { bg: 'bg-red-50', text: 'text-red-600', border: 'border-red-200' }, medium: { bg: 'bg-amber-50', text: 'text-amber-600', border: 'border-amber-200' }, low: { bg: 'bg-blue-50', text: 'text-blue-600', border: 'border-blue-200' }, };
 ...
 return (
  <motion.div className="space-y-6" variants={containerVariants} initial="hidden" animate="visible">
   ...
   <motion.div variants={itemVariants}>
    <Card className="border-0 shadow-xl overflow-hidden relative rounded-3xl" style={{ background: 'linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%)' }}>
     <div className="absolute top-0 right-0 w-72 h-72 rounded-full opacity-20 -mr-16 -mt-16" style={{ background: `linear-gradient(135deg, ${COLORS.deepBlue} 0%, ${COLORS.mediumBlue} 100%)` }} />
     <CardContent className="p-8 relative">
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
       ...
       {nextDeadline && (
        <motion.div whileHover={{ scale: 1.02, y: -2, transition: springPhysics.card }} className="flex items-center gap-5 px-6 py-4 rounded-3xl border-2 cursor-pointer hover:shadow-2xl transition-all" style={{ borderColor: COLORS.mediumBlue, backgroundColor: 'white' }} onClick={() => navigate('/duedates')}>
   ...
   <motion.div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4" variants={itemVariants}>
    {/* Total Tasks */}
    <motion.div whileHover={{ y: -5, scale: 1.01, transition: springPhysics.card }} whileTap={{ scale: 0.985 }} onClick={() => navigate('/tasks')} className="border border-slate-100 shadow-sm hover:shadow-2xl hover:border-slate-200 transition-all cursor-pointer group rounded-3xl">
    ...
    {/* Overdue Tasks */}
    <motion.div whileHover={{ y: -5, scale: 1.01 }} whileTap={{ scale: 0.985 }} onClick={() => navigate('/tasks?filter=overdue')} className={`border shadow-sm hover:shadow-2xl transition-all cursor-pointer group rounded-3xl ${stats?.overdue_tasks > 0 ? 'border-red-200 bg-red-50/50' : 'border-slate-100'}`}>
     <CardContent className="p-6 flex flex-col h-full">
      <div className="flex items-start justify-between flex-1">
       <div>
    ...
    {/* Completion Rate */}
    <motion.div whileHover={{ y: -5, scale: 1.01 }} whileTap={{ scale: 0.985 }} onClick={() => navigate('/tasks')} className="border border-slate-100 shadow-sm hover:shadow-2xl hover:border-slate-200 transition-all cursor-pointer group rounded-3xl">
     <CardContent className="p-6 flex flex-col h-full">
      <div className="flex items-start justify-between flex-1">
       <div>
    ...
    {/* DSC Alerts */}
    <motion.div whileHover={{ y: -5, scale: 1.01 }} whileTap={{ scale: 0.985 }} onClick={() => navigate('/dsc?tab=expired')} className={`border shadow-sm hover:shadow-2xl transition-all cursor-pointer group rounded-3xl ${stats?.expiring_dsc_count > 0 ? 'border-red-200 bg-red-50/50' : 'border-slate-100'}`}>
     <CardContent className="p-6 flex flex-col h-full">
      <div className="flex items-start justify-between flex-1">
       <div>
    ...
    {/* Today's Attendance */}
    <motion.div whileHover={{ y: -5, scale: 1.01 }} whileTap={{ scale: 0.985 }} onClick={() => navigate('/attendance')} className="border border-slate-100 shadow-sm hover:shadow-2xl hover:border-slate-200 transition-all cursor-pointer group rounded-3xl">
     <CardContent className="p-6 flex flex-col h-full">
      <div className="flex items-start justify-between flex-1">
       <div>
       ...
       {todayAttendance?.punch_in ? (
        <>
         ...
         {todayAttendance.punch_out ? (
          ...
          <Button onClick={() => handlePunchAction('punch_out')} className="w-full bg-red-600 hover:bg-red-700 rounded-2xl" disabled={loading}>
           Punch Out
          </Button>
         )}
        ...
        <Button onClick={() => handlePunchAction('punch_in')} className="w-full bg-green-600 hover:bg-green-700 rounded-2xl" disabled={loading}>
         Punch In
        </Button>
       )}
      </div>
   ...
   {showTaskSection && (
    <motion.div variants={itemVariants} className="grid grid-cols-1 xl:grid-cols-2 gap-4">
     ...
     <Card className="flex flex-col border-slate-100 shadow-sm rounded-3xl overflow-hidden cursor-pointer hover:shadow-xl transition group" onClick={() => navigate('/tasks?filter=assigned-to-me')}>
      <CardHeader className="pb-4 border-b px-6">
       <div className="flex items-center justify-between">
        ...
        <Button variant="ghost" size="sm" onClick={e => { e.stopPropagation(); navigate('/tasks?filter=assigned-to-me'); }} className="text-emerald-600 hover:text-emerald-700">
         View All →
        </Button>
       </div>
        ...
        <div className="space-y-4 max-h-[360px] overflow-y-auto pr-2">
         <AnimatePresence>
          {tasksAssignedToMe.map(task => (
           <TaskStrip key={task.id} task={task} isToMe={true} assignedName={task.assigned_by_name || task.created_by_name || 'Unknown'} onUpdateStatus={updateAssignedTaskStatus} navigate={navigate} />
          ))}
         </AnimatePresence>
        </div>
       )}
     ...
     <Card className="flex flex-col border-slate-100 shadow-sm rounded-3xl overflow-hidden cursor-pointer hover:shadow-xl transition group" onClick={() => navigate('/tasks?filter=assigned-by-me')}>
      <CardHeader className="pb-4 border-b px-6">
       <div className="flex items-center justify-between">
        <CardTitle className="text-xl font-semibold flex items-center gap-3">
        ...
        <Button variant="ghost" size="sm" onClick={e => { e.stopPropagation(); navigate('/tasks?filter=assigned-by-me'); }} className="text-blue-600 hover:text-blue-700">
         View All →
        </Button>
       </div>
        ...
        <div className="space-y-4 max-h-[360px] overflow-y-auto pr-2">
         <AnimatePresence>
          {tasksAssignedByMe.map(task => (
           <TaskStrip key={task.id} task={task} isToMe={false} assignedName={task.assigned_to_name || 'Unknown'} onUpdateStatus={updateAssignedTaskStatus} navigate={navigate} />
          ))}
         </AnimatePresence>
        </div>
       )}
       ...
       {isAdmin && (
        <div className="flex gap-1">
         {["all", "monthly", "weekly"].map(p => (
          <Button key={p} variant={rankingPeriod === p ? "default" : "outline"} size="sm" onClick={() => setRankingPeriod(p)} className="text-xs px-3 py-1">
           {p.toUpperCase()}
          </Button>
         ))}
        </div>
     ...
     <CardContent className="p-6">
      ...
      {todos.length === 0 ? (
       ...
       <div className="space-y-4 max-h-[360px] overflow-y-auto pr-2">
        <AnimatePresence>
         {todos.map(todo => (
          <motion.div key={todo._id || todo.id} variants={itemVariants} className={`flex items-center justify-between gap-4 p-5 rounded-3xl border ${todo.completed ? 'bg-green-50 border-green-200' : 'bg-slate-50 border-slate-200'} ${!todo.completed && isOverdue(todo.due_date) ? 'border-red-400 bg-red-50/60' : ''}`}>
           <div className="flex items-center gap-4 flex-1 min-w-0">
            <input type="checkbox" checked={todo.completed} onChange={() => handleToggleTodo(todo._id || todo.id)} className="h-5 w-5 accent-emerald-600 flex-shrink-0" />
            <div className="flex-1 min-w-0">
             <span className={`block text-sm ${todo.completed ? 'line-through text-slate-500' : 'text-slate-900'}`}>
              {todo.title} {!todo.completed && isOverdue(todo.due_date) && ( <span className="ml-2 px-2 py-0.5 text-xs font-medium bg-red-100 text-red-700 rounded-full"> Overdue </span> )}
             </span>
             ...
             {todo.due_date && (
              <p className={`text-xs mt-0.5 ${isOverdue(todo.due_date) ? 'text-red-600 font-medium' : 'text-amber-600'}`}>
               Due: {format(new Date(todo.due_date), 'MMM d, yyyy')} {isOverdue(todo.due_date) && ' (overdue)'}
              </p>
             )}
            </div>
           </div>
           <Button variant="destructive" size="sm" onClick={() => handleDeleteTodo(todo._id || todo.id)}>
            Delete
           </Button>
          </motion.div>
         ))}
   ...
   <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4" variants={itemVariants}>
    <motion.div whileHover={{ y: -5, scale: 1.01 }} whileTap={{ scale: 0.985 }} onClick={() => navigate('/leads')} className="border border-slate-100 hover:shadow-xl hover:border-slate-200 transition-all cursor-pointer group rounded-3xl">
    ...
    <motion.div whileHover={{ y: -5, scale: 1.01 }} whileTap={{ scale: 0.985 }} onClick={() => navigate('/clients')} className="border border-slate-100 hover:shadow-xl hover:border-slate-200 transition-all cursor-pointer group rounded-3xl">
     <CardContent className="p-6 flex items-center gap-5">
    ...
    <motion.div whileHover={{ y: -5, scale: 1.01 }} whileTap={{ scale: 0.985 }} onClick={() => navigate('/dsc')} className="border border-slate-100 hover:shadow-xl hover:border-slate-200 transition-all cursor-pointer group rounded-3xl">
     <CardContent className="p-6 flex items-center gap-5">
    ...
    <motion.div whileHover={{ y: -5, scale: 1.01 }} whileTap={{ scale: 0.985 }} onClick={() => navigate('/duedates')} className="border border-slate-100 hover:shadow-xl hover:border-slate-200 transition-all cursor-pointer group rounded-3xl">
     <CardContent className="p-6 flex items-center gap-5">
    ...
    {isAdmin && (
     <motion.div whileHover={{ y: -5, scale: 1.01 }} whileTap={{ scale: 0.985 }} onClick={() => navigate('/users')} className="border border-slate-100 hover:shadow-xl hover:border-slate-200 transition-all cursor-pointer group rounded-3xl">
      <CardContent className="p-6 flex items-center gap-5">
       <div className="p-4 rounded-2xl group-hover:scale-125 transition-transform" style={{ backgroundColor: `${COLORS.mediumBlue}15` }}>
...
<AnimatePresence>
 {mustPunchIn && (
  <motion.div className="fixed inset-0 z-[9999] bg-black/60 backdrop-blur-md flex items-center justify-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
   <motion.div initial={{ scale: 0.9, y: 40 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 40 }} transition={{ type: "spring", stiffness: 160, damping: 18 }} className="bg-white w-full max-w-md mx-4 p-6 md:p-10 rounded-3xl shadow-2xl text-center relative">
    <motion.h2 className="text-3xl font-bold mb-3" initial={{ scale: 0.95 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 220, damping: 14 }}>
     {getGreeting()}
    </motion.h2>
...
<motion.div initial={{ y: 0 }} animate={{ y: [0, -2, 0] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }} whileHover={{ y: 0 }}>
 <Button onClick={async () => { await handlePunchAction('punch_in'); setMustPunchIn(false); document.body.style.overflow = "auto"; }} disabled={loading} className="w-full h-12 text-lg bg-green-600 hover:bg-green-700 rounded-2xl shadow-lg hover:shadow-xl transition-all">
  {loading ? "Punching In..." : "Punch In"}
 </Button>
</motion.div>
<div className="mt-4">
 <Button variant="secondary" className="w-full" onClick={async () => { setLoading(true); try { await api.post("/attendance/mark-leave-today"); toast.success("Marked on leave today"); queryClient.invalidateQueries({ queryKey: ['todayAttendance'] }); setMustPunchIn(false); document.body.style.overflow = "auto"; } catch (err) { toast.error("Failed to mark leave"); } finally { setLoading(false); } }}>
  On Leave Today
 </Button>
</div>
   </motion.div>
