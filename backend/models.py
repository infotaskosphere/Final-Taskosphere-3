...
class User(BaseModel):
    ...
    def can_view_resource(
        self,
        owner_user_id: str,
        cross_user_list_key: Optional[str] = None,
    ) -> bool:
        if self.is_admin:
            return True
        ...
        if not cross_user_list_key:
            return False

        cross_list: List[str] = getattr(self.permissions, cross_user_list_key, []) or []

        if "*" in cross_list:
            return True

        return owner_user_id in cross_list
    def can_view_attendance_of(self, target_user_id: str) -> bool:
        if self.is_admin:
            return True

        if not self.permissions.can_view_attendance:
            return False

        if self.id == target_user_id:
            return True

        allowed_users = self.permissions.view_other_attendance or []

        if "*" in allowed_users:
            return True

        return target_user_id in allowed_users
    def can_view_task_of(self, target_user_id: str) -> bool:
        if self.is_admin:
            return True

        if not self.permissions.can_view_all_tasks and self.id != target_user_id:
            allowed_users = self.permissions.view_other_tasks or []
        ...
        return True
    def can_view_visit_of(self, target_user_id: str) -> bool:
        if self.is_admin:
            return True

        if self.permissions.can_view_all_visits:
            return True

        if self.id == target_user_id:
            return True

        allowed_users = self.permissions.view_other_visits or []
