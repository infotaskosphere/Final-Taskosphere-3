...
async def get_leads(
    status_filter: Optional[
        Literal["new", "contacted", "qualified", "won", "lost"]
...
async def update_lead(
 ...
 if current_user.role != "admin":
     if existing.get("assigned_to") != current_user.id and existing.get("created_by") != current_user.id:
         raise HTTPException(
             status_code=403,
...
async def convert_lead_to_client(
 ...
 if current_user.role != "admin":
     if lead.get("assigned_to") != current_user.id and lead.get("created_by") != current_user.id:
         raise HTTPException(
             status_code=403,
...
async def delete_lead(lead_id: str, current_user=Depends(get_current_user)):
 ...
 if not permissions.get("can_manage_users", False) and current_user.role != "admin":
 raise HTTPException(
 status_code=403,
