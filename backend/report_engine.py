...
# Statuses considered "blocking" for new registration
...
def _phonetic_keys(s: str) -> tuple:
    """Double Metaphone phonetic codes (primary, alternate)."""
...
def _phonetic_match(a: str, b: str) -> bool:
    """True if two strings share at least one Double Metaphone code."""
...
def build_report(query: str, scraped: Dict, class_filter: Optional[int] = None) -> Dict:
    ...
    if blocking_exact:
        ...
        headline = (
            "Largely clear — only weak or expired matches found. "
            "Filing this trademark looks feasible."
    ...
    # Risk score: weighted aggregation
    if enriched:
    ...
    if overall == "CONFLICT":
        recommendations.append(
            "Do NOT file as-is. The identical mark already exists in a blocking status."
        )
        ...
        recommendations.append(
            "Consider filing in a class with fewer existing conflicts (see class breakdown)."
        )
        recommendations.append(
            "Engage a trademark attorney to draft a strong specification of goods/services."
        )
    else:
        recommendations.append("Proceed with filing — risk profile is low.")
        recommendations.append(
            "Lock in the mark quickly: TM registration in India operates on a first-to-file basis."
        )
