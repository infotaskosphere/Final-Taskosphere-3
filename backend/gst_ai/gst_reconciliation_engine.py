...
def _extract_b2b_records_from_raw(raw, seen_keys=None):
    """Shared B2B-table reader used by every input format (Excel sheet, PDF
    table, or a sheet/table pulled out of a ZIP). `raw` is a header-less,
    all-string DataFrame that contains the two-row GSTR-2B B2B header
    somewhere near the top followed by data rows — exactly what
    `pd.read_excel(..., header=None, dtype=str)` or a pdfplumber table
    (wrapped in `pd.DataFrame`) produces.

    `seen_keys` may be passed in (and mutated) by callers that need
    duplicate-detection to span multiple tables/pages/files — e.g. a PDF
    with the B2B table split across pages, or a ZIP of monthly exports.
    """
...
def _parse_portal(file_bytes, filename):
    """Read a single GSTR-2B Excel export (.xlsx/.xls) downloaded from the
    GST portal and return a DataFrame of B2B invoice records."""
    ext    = filename.rsplit(".", 1)[-1].lower()
    engine = "xlrd" if ext == "xls" else "openpyxl"
    try:
        xl = pd.ExcelFile(io.BytesIO(file_bytes), engine=engine)
    ...
    raw = pd.read_excel(xl, sheet_name=sheet, header=None, dtype=str)
    records = _extract_b2b_records_from_raw(raw)
    return pd.DataFrame(records)


def _parse_portal_pdf(file_bytes, filename):
    """Read a GSTR-2B PDF export downloaded from the GST portal and return a
    DataFrame of B2B invoice records. The PDF renders the same B2B table as
    the Excel export (repeated on every page it spans), so every extractable
    table on every page is checked and, if it looks like a B2B table (has a
    'GSTIN' column near the top), run through the same row-extraction logic
    used for Excel. A single `seen_keys` set is shared across pages so a
    header row repeated at the top of each page doesn't cause a real
    duplicate to be miscounted, and so a table genuinely split across a page
    break is still deduplicated correctly.
    """
    try:
        import pdfplumber
    except ImportError:
        raise HTTPException(500, "pdfplumber is not installed on the server; cannot read PDF exports.")
            ...
            for page in pdf.pages:
                for table in (page.extract_tables() or []):
                    if not table or len(table) < 2:
                        continue
                    header_blob = " ".join(_to_str(c) for row in table[:6] for c in row).lower()
                    if "gstin" not in header_blob:
                        continue  # not a B2B-style table (e.g. a summary/ITC table on the same page)
                    raw = pd.DataFrame(table)
                    records.extend(_extract_b2b_records_from_raw(raw, seen_keys=seen_keys))
    except HTTPException:
        raise
    except Exception as exc:
        raise HTTPException(400, f"Cannot open GSTR-2B PDF: {exc}")

    logger.info("Portal PDF '%s': parsed %d invoices", filename, len(records))
    return pd.DataFrame(records)


def _parse_portal_zip(file_bytes, filename):
    """Read a ZIP archive containing one or more GSTR-2B exports (any mix of
    .xlsx/.xls/.pdf — e.g. a year's worth of monthly downloads from the GST
    portal, zipped together) and return a single combined DataFrame of B2B
    invoice records across every file in the archive.
    """
    import zipfile
    ...
    try:
        ...
        raise HTTPException(400, f"Cannot open ZIP file: {exc}")
    ...
    members = [
        ...
        and n.rsplit(".", 1)[-1].lower() in ("xlsx", "xls", "pdf")
    ]
    if not members:
        raise HTTPException(400, "This ZIP doesn't contain any GSTR-2B Excel or PDF files.")
    ...
    for name in members:
        ...
        try:
            ...
            df = _parse_portal_pdf(data, short_name) if ext == "pdf" else _parse_portal(data, short_name)
            if df is not None and not df.empty:
                df = df.copy()
                df["source_file"] = short_name
                frames.append(df)
        except HTTPException as exc:
            errors.append(f"{short_name}: {exc.detail}")
        except Exception as exc:
            errors.append(f"{short_name}: {exc}")

    if not frames:
        detail = "No B2B invoices could be read from any file inside the ZIP."
        if errors:
            detail += " (" + "; ".join(errors[:5]) + ")"
        raise HTTPException(400, detail)

    logger.info("Portal ZIP '%s': %d/%d files parsed, %d skipped/failed", filename, len(frames), len(members), len(errors))
    return pd.concat(frames, ignore_index=True)


def _parse_portal_any(file_bytes, filename):
    """Dispatch a GST-portal upload (Excel, PDF, or ZIP of either) to the
    right reader and return a combined B2B invoice DataFrame. This is the
    entry point new callers should use instead of `_parse_portal` directly,
    so they transparently support every upload format."""
    ext = filename.rsplit(".", 1)[-1].lower() if "." in filename else ""
    if ext in ("xlsx", "xls"):
        return _parse_portal(file_bytes, filename)
    if ext == "pdf":
        return _parse_portal_pdf(file_bytes, filename)
    if ext == "zip":
        return _parse_portal_zip(file_bytes, filename)
    raise HTTPException(
        400,
        "Unsupported file type. Please upload the GSTR-2B Excel (.xlsx/.xls) or PDF export from "
        "the GST portal, or a ZIP file containing one or more of them.",
    )
