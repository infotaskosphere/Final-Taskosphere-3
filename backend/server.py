...
# IN-MEMORY CACHE for daily reminder
...
def mark_absent_users_task():
    """Sync wrapper called by APScheduler at 19:00 IST."""
    loop = None
    try:
...
async def startup():
    ...
    try:
        ...
        logger.info("Non-unique indexes created successfully.")
    except Exception as e:
        logger.warning(f"Non-unique index creation warning (non-fatal): {e}")

    # ── UNIQUE indexes (each in its own try/except so one failure doesn't block others) ──
    ...
    try:
        ...
        logger.warning(f"attendance unique index (non-fatal): {e}")

    # clients unique index
    try:
        ...
        logger.warning(f"clients unique index (non-fatal): {e}")

    # holidays unique index
    try:
        await db.holidays.create_index("date", unique=True, background=True)
    except Exception as e:
        logger.warning(f"holidays unique index (non-fatal): {e}")

    # ── FIX: email_connections index created ONCE (was duplicated before) ────
    # Drop the old provider-based index first (best-effort), then create the
    # correct email_address-based unique index exactly once.
    try:
    ...
    try:
        ...
        logger.warning(f"email_connections unique index (non-fatal): {e}")
    ...
    try:
        ...
        logger.error(f"Visit ID repair failed (non-fatal): {e}")
...
def fetch_indian_holidays_task():
    """Scheduled job (sync wrapper) to fetch holidays for the current month."""
    loop = None
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
...
def send_birthday_email(recipient_email: str, client_name: str):
    ...
    May this year bring you prosperity, success, and happiness.
    </p>
...
async def get_todos(
    ...
    if current_user.role == "admin":
        ...
        allowed_others = (
            permissions.get("view_other_todos", [])
...
async def update_todo(
    ...
    await db.todos.update_one({"_id": todo["_id"]}, {"$set": updates})
...
async def create_reminder(
    ...
    if isinstance(raw, datetime):
        remind_dt = raw if raw.tzinfo else raw.replace(tzinfo=IST_tz)
    else:
        ...
        if not remind_dt:
            for fmt in [
                ...
                try:
                    remind_dt = datetime.strptime(raw_str, fmt).replace(tzinfo=IST_tz)
                    break
                except ValueError:
                    continue
        if not remind_dt:
            remind_dt = (datetime.now(IST_tz) + timedelta(days=1)).replace(
...
async def get_attendance_history(
    ...
    if current_user.role == "admin":
        ...
        if user_id:
            if user_id == current_user.id:
                ...
                if not permissions_mgr.get("can_view_attendance", False):
                    raise HTTPException(status_code=403, detail="No permission to view other attendance")
                if user_id not in allowed_users:
                    raise HTTPException(status_code=403, detail="User outside visibility scope")
                query["user_id"] = user_id
        else:
        ...
        if user_id and user_id != current_user.id:
            ...
            if user_id not in allowed_users:
                raise HTTPException(status_code=403, detail="Not authorized to view other attendance")
...
async def get_staff_attendance_report(
    ...
    for uid, data in staff_report.items():
        ...
        data["avg_hours_per_day"] = (
            round((total_minutes / data["days_present"]) / 60, 1)
...
async def get_top_performers_data(period: str = "monthly", limit: int = 5, db=None):
    ...
    pipeline = [
        ...
        {"$lookup": {"from": "users", "localField": "_id", "foreignField": "id", "as": "user_info"}},
        {"$unwind": "$user_info"},
        {
            "$project": {
                "user_id": "$_id",
...
async def create_task(
    ...
    if (task_data.assigned_to
        ...
        if not perms.get("can_assign_tasks", False):
            raise HTTPException(status_code=403, detail="No permission to assign tasks to others")
    task_id = str(uuid.uuid4())
    now = datetime.now(timezone.utc)
    task = Task(**task_data.model_dump(), id=task_id, created_by=current_user.id, created_at=now, updated_at=now)
    doc = task.model_dump()
    for field in ["created_at", "updated_at", "due_date", "recurrence_end_date"]:
    ...
    await create_audit_log(
        current_user=current_user, action="CREATE_TASK", module="tasks",
        record_id=task_id, new_data={"title": task.title}
    )
    return task
...
async def patch_task(
    ...
    await create_audit_log(
        current_user=current_user, action=action_type, module="task",
        record_id=task_id, old_data=old_data, new_data=updates
    )
...
async def delete_task(task_id: str, current_user: User = Depends(get_current_user)):
    ...
    await create_audit_log(
        current_user=current_user, action="DELETE_TASK", module="task",
        record_id=task_id, old_data=existing
    )
...
async def export_task_log_pdf(
    ...
    pdf.multi_cell(0, 7, f"Status: {task.get('status', '-')}")
    pdf.ln(8)
    pdf.set_font("Arial", "B", 12)
...
async def create_dsc(
    ...
    try:
        now = datetime.now(timezone.utc)
        dsc = DSC(**dsc_data.model_dump(), created_by=current_user.id, created_at=now)
        doc = dsc.model_dump()
...
COMPLIANCE_RULES = [
    ...
    {"keywords": ["tds", "tax deducted at source", "form 24q", "form 26q"],            "category": "TDS",        "department": "TDS"},
    ...
    {"keywords": ["dir-3", "director kyc"],                                            "category": "ROC",        "department": "ROC"},
    {"keywords": ["roc", "mca", "companies act"],                                      "category": "ROC",        "department": "ROC"},
    {"keywords": ["msme"],                                                             "category": "Other",      "department": "MSME"},
    {"keywords": ["trademark", "tm renewal"],                                          "category": "Trademark",  "department": "TM"},
    {"keywords": ["fema", "foreign exchange"],                                         "category": "FEMA",       "department": "FEMA"},
...
# Route registered BEFORE /{due_date_id} param routes
...
async def extract_due_dates_from_file(
    ...
    try:
        if content_type.startswith("image/") or filename.endswith((".jpg", ".jpeg", ".png", ".webp", ".bmp")):
            raise HTTPException(status_code=400, detail="Image upload not supported. Use PDF or DOCX.")
    ...
    if not extracted:
        raise HTTPException(status_code=404, detail="No compliance dates detected.")
...
async def get_efficiency_report(
    ...
    if target_user_id != current_user.id:
        ...
        if current_user.role != "admin":
            ...
            if target_user_id not in allowed_users:
                raise HTTPException(status_code=403, detail="Not authorized to view other reports")
    ...
    return {
        ...
        "user": {"id": target_user_id, "full_name": target_user_doc.get("full_name", "Unknown") if target_user_doc else "Unknown"},
...
async def export_reports(
    ...
    if current_user.role != "admin" and not perms.get("can_download_reports", False):
        raise HTTPException(status_code=403, detail="No permission to download reports")
    target_user_id = user_id or current_user.id
    if target_user_id != current_user.id and current_user.role != "admin":
        permissions = get_user_permissions(current_user)
        allowed_users = permissions.get("view_other_reports", [])
        if target_user_id not in allowed_users:
            raise HTTPException(status_code=403, detail="Not authorized to access other reports")
...
async def get_performance_rankings(
    ...
    for user in users:
        ...
        attendance_pct  = round((days_present / expected_working_days) * 100, 1) if expected_working_days else 0
        timely_days     = len([r for r in att_records if not r.get("is_late", False)])
        timely_pct      = round((timely_days / days_present) * 100, 1) if days_present else 0
        ...
        completed_tasks = await db.tasks.count_documents({
            ...
            "$or": [{"completed_at": {"$gte": start_date}}, {"updated_at": {"$gte": start_date}}]
        })
        ...
        task_completion_pct = round((total_completed / tasks_assigned) * 100, 1) if tasks_assigned else 0
        ...
        for t in todos:
            if t.get("is_completed"):
                ...
                if due and completed_at and completed_at <= due:
                    completed_ontime += 1
        todo_ontime_pct = round((completed_ontime / len(todos)) * 100, 1) if todos else 0
        safe_hours_ratio = min((total_hours / 180), 1) if total_hours else 0
        score = (
            float(attendance_pct)     * 0.25 +
            safe_hours_ratio * 100    * 0.20 +
            float(task_completion_pct)* 0.25 +
            float(todo_ontime_pct)    * 0.15 +
            float(timely_pct)         * 0.15
        )
        overall_score = round(min(score, 100), 1)
        badge = (
            "Star Performer"  if overall_score >= 95 else
            "Top Performer"   if overall_score >= 85 else
            "Good Performer"
        )
        rankings.append(PerformanceMetric(
            ...
            attendance_percent=float(attendance_pct),
            total_hours=float(total_hours),
            task_completion_percent=float(task_completion_pct),
            todo_ontime_percent=float(todo_ontime_pct),
            timely_punchin_percent=float(timely_pct),
...
async def import_master_data_preview(
    ...
    if not filename.endswith((".xlsx", ".xls")):
        raise HTTPException(status_code=400, detail="Only Excel formats supported.")
    try:
        content = await file.read()
        ...
        for sheet_name in excel.sheet_names:
            df = pd.read_excel(excel, sheet_name=sheet_name).fillna("")
            records = df.to_dict(orient="records")
            parsed_blueprint[sheet_name] = records
            total_vectors += len(records)
        return {
            "status": "Ready for Audit",
            "message": f"Detected {len(excel.sheet_names)} sheets with {total_vectors} rows.",
...
async def sync_master_sheets(
    ...
    try:
            ...
            if "client" in sheet_type:
                for rec in records:
                    await db.clients.update_one(
                        ...
                        {"$set": {**rec, "id": str(uuid.uuid4()), "created_by": current_user.id, "updated_at": now_iso}},
                        upsert=True
                    )
        ...
        await create_audit_log(current_user, "GLOBAL_MASTER_SYNC", "master_data", "multi_sheet_payload", new_data=sync_results)
        return {"message": "Master Sync Executed", "telemetry": sync_results}
    except Exception as e:
...
async def parse_mds_excel_for_client_form(
    ...
    for sheet_name in excel.sheet_names:
        ...
        if "master" in sheet_lower or "company" in sheet_lower:
            for _, row in df.iterrows():
                key   = str(row.iloc[0]).strip()
                value = str(row.iloc[1]).strip() if len(row) > 1 else ""
                if key and key not in ("", "nan") and value not in ("", "nan"):
                    company_info[key] = value
        elif "director" in sheet_lower or "partner" in sheet_lower:
            ...
            for row in rows_list[header_row_idx + 1:]:
                ...
                directors.append({
                    "name": name,
                    "designation": row_dict.get("Designation", "Director"),
                    "email": None, "phone": None, "birthday": None,
                    "din": din if din not in ("nan", "-", "") else None,
                })

    company_name = (company_info.get("Company Name") or company_info.get("LLP Name") or "").strip()
    email   = clean_email(company_info.get("Email Id") or company_info.get("Email") or "")
    phone   = clean_phone(company_info.get("Phone") or company_info.get("Mobile") or "")
    birthday = parse_date_str(company_info.get("Date of Incorporation") or "")
    address  = (company_info.get("Registered Address") or company_info.get("Address") or "").strip()
    if address in ("-", "nan"):
        address = ""
    status_raw = (company_info.get("Company Status") or company_info.get("LLP Status") or "Active").lower()
    return {
        "status": "ok",
        ...
        "services": [], "notes": "",
        "status_value": "active" if "active" in status_raw else "inactive",
        "contact_persons": directors, "dsc_details": [],
...
async def create_client(payload: dict, current_user: User = Depends(get_current_user)):
    try:
        ...
        for key in ("address", "city", "state", "client_type_label", "contact_persons", "dsc_details", "assignments", "referred_by"):
            val = payload.get(key)
            if val is not None:
                doc[key] = val
        doc.pop("_id", None)
...
async def update_client(
    ...
    ALLOWED_FIELDS = {
        ...
        "address", "city", "state", "services", "notes",
        "assigned_to", "assignments", "status", "contact_persons",
        "dsc_details", "referred_by",
    }
    ...
    VALID_CLIENT_TYPES = {"proprietor", "pvt_ltd", "llp", "partnership", "huf", "trust", "other", "LLP", "PVT_LTD"}
    if "client_type" in update_data:
        ct = update_data["client_type"]
        if ct not in VALID_CLIENT_TYPES:
            raise HTTPException(status_code=422, detail=f"Invalid client_type '{ct}'")
    ...
    if "company_name" in update_data:
        ...
        if len(name) < 3:
            raise HTTPException(status_code=422, detail="Company name must be at least 3 characters")
    ...
    if update_data.get("phone"):
        ...
        if not (10 <= len(cleaned_phone) <= 15):
            raise HTTPException(status_code=422, detail="Phone must be 10-15 digits")
...
async def delete_client(client_id: str, current_user: User = Depends(get_current_user)):
    ...
    if current_user.role != "admin" and not perms.get("can_delete_data", False):
        raise HTTPException(status_code=403, detail="No permission to delete clients")
    existing = await db.clients.find_one({"id": client_id}, {"_id": 0})
    if not existing:
        raise HTTPException(status_code=404, detail="Client not found")
    await db.leads.update_many({"converted_client_id": client_id}, {"$set": {"converted_client_id": None}})
...
async def get_dashboard_stats(current_user: User = Depends(get_current_user)):
        ...
        try:
            ...
            if days_left <= 90:
                ...
                expiring_dsc_list.append({
                    "id": dsc["id"], "holder_name": dsc["holder_name"],
                    "expiry_date": dsc["expiry_date"], "days_left": days_left,
    ...
    return DashboardStats(
        ...
        total_dsc=total_dsc, expiring_dsc_count=expiring_dsc_count,
        expiring_dsc_list=expiring_dsc_list, total_clients=total_clients,
        upcoming_birthdays=upcoming_birthdays, upcoming_due_dates=upcoming_due_dates_count,
        team_workload=team_workload, compliance_status=compliance_status,
...
# FIX: Use asyncio.create_task() instead of deprecated asyncio.ensure_future()
# ─────────────────────────────────────────────────────────────────────────────
async def _run_daily_reminder_job(today_str: str):
    ...
    try:
        ...
        if db_last_date != today_str:
            logger.info("Auto daily reminder triggered")
...
async def auto_daily_reminder(request: Request, call_next):
    ...
    try:
        ...
        if india_time.hour >= 10 and _last_reminder_date_cache != today_str:
            ...
            # FIX: asyncio.create_task() instead of deprecated ensure_future()
...
async def create_holiday(
    ...
    if existing:
        ...
        return await db.holidays.find_one({"date": holiday_dict["date"]}, {"_id": 0})
    try:
...
def _extract_holidays_from_text(raw_text: str) -> list:
    ...
    default_year = int(Counter(year_matches).most_common(1)[0][0]) if year_matches else datetime.now().year
    lines = [l.strip() for l in raw_text.splitlines() if l.strip()]
    for line in lines:
        if re.match(r'^(sl\.?\s*no|s\.?\s*no|date|day|holiday|occasion|name|month)', line, re.IGNORECASE):
            continue
        if len(line) < 5:
            continue
...
async def extract_holidays_from_pdf(
    ...
    if not holidays:
        raise HTTPException(status_code=404, detail="No holidays detected in PDF.")
    logger.info(f"PDF holiday extraction: {len(holidays)} holidays found by {current_user.email}")
    return {"holidays": holidays}


# ─────────────────────────────────────────────────────────────────────────────
# TRADEMARK NOTICE PDF EXTRACTOR
# ─────────────────────────────────────────────────────────────────────────────
...
def _extract_trademark_notice_data(raw_text: str) -> dict:
    result = {
        "document_type": None, "application_no": None, "class": None,
        "application_date": None, "used_since": None, "applicant_name": None,
        "recipient_name": None, "hearing_date": None, "letter_date": None,
        "brand_name": None, "raw_text_snippet": raw_text[:500].strip(),
    }
    text_lower = raw_text.lower()
    if "show cause" in text_lower:
    ...
    m = re.search(r'(?:in\s+class(?:es)?|class(?:es)?)[:\s\/]*(\d{1,2}(?:\s*[,&]\s*\d{1,2})*)', raw_text, re.IGNORECASE)
    if m:
        result["class"] = m.group(1).strip()
    m = re.search(r'(?:hearing\s+on|fixed\s+for\s+hearing\s+on)\s+(\d{1,2}[-\/]\d{1,2}[-\/]\d{4})', raw_text, re.IGNORECASE)
    if m:
        result["hearing_date"] = _parse_tm_date(m.group(1))
    m = re.search(r'(?:dated?|दिनांक)[:\s]*(\d{1,2}[-\/]\d{1,2}[-\/]\d{4})', raw_text, re.IGNORECASE)
    if m:
...
async def extract_trademark_notice(
    ...
    if not extracted["application_no"] and not extracted["hearing_date"]:
        raise HTTPException(status_code=404, detail="Could not find application number or hearing date.")
    return extracted


# ─────────────────────────────────────────────────────────────────────────────
# GLOBAL EXCEPTION HANDLER — includes CORS headers on 500 responses
# ─────────────────────────────────────────────────────────────────────────────
