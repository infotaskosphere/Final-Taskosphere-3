...
def sanitize_user_data(user_data, current_user):
    ...
    if isinstance(user_data, list):
        return [to_dict(item) for item in user_data]
...
def check_permission(permission_name: str):
    def dependency(current_user: User = Depends(get_current_user)):
        if current_user.role == "admin":
            return current_user
        # ✅ Handle cases where permissions might be None in DB
        user_perms = current_user.permissions.model_dump() if current_user.permissions else {}
        if not user_perms.get(permission_name, False):
            raise HTTPException(status_code=403, detail="Access denied: Missing permission")
        return current_user
    return dependency
app = FastAPI()
@app.get("/health")
...
async def get_me(current_user: User = Depends(get_current_user)):
    ...
    if hasattr(current_user.created_at, 'isoformat'):
        user_info["created_at"] = current_user.created_at.isoformat()
    else:
        user_info["created_at"] = current_user.created_at
...
async def get_staff_rankings(
    ...
    # ✅ Sort ONLY after map is fully built
    sorted_users = sorted(ranking_map.items(), key=lambda x: x[1], reverse=True)
    # 🔥 Fetch user names
