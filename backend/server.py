...
# ─────────────────────────────────────────────────────────────────────────────
# ATTENDANCE LATE / EARLY-OUT HELPERS
# ─────────────────────────────────────────────────────────────────────────────

def _parse_hhmm(hhmm: str) -> tuple[int, int]:
    """Parse a 'HH:MM' string into (hours, minutes). Returns (0,0) on failure."""
    try:
...
def check_is_late(user: User, punch_in_ist: datetime) -> bool:
    """
    Returns True when the user punched in AFTER their allowed window.

    Window = punch_in_time + grace_time
    Both fields are HH:MM strings stored on the User document.
    punch_in_ist must be an IST-aware datetime.
    """
    try:
        pi_h, pi_m = _parse_hhmm(user.punch_in_time or "10:30")
        gr_h, gr_m = _parse_hhmm(user.grace_time    or "00:15")

        deadline = datetime(
            punch_in_ist.year, punch_in_ist.month, punch_in_ist.day,
            pi_h, pi_m, 0,
            tzinfo=ZoneInfo("Asia/Kolkata")
        ) + timedelta(hours=gr_h, minutes=gr_m)

        return punch_in_ist > deadline
    except Exception as e:
        logger.warning(f"check_is_late failed for user {user.id}: {e}")
        return False


def check_punched_out_early(user: User, punch_out_ist: datetime) -> bool:
    """
    Returns True when the user punched out BEFORE their configured punch_out_time.

    punch_out_time is an HH:MM string stored on the User document.
    punch_out_ist must be an IST-aware datetime.
    """
    try:
        po_h, po_m = _parse_hhmm(user.punch_out_time or "19:00")

        expected_out = datetime(
            punch_out_ist.year, punch_out_ist.month, punch_out_ist.day,
            po_h, po_m, 0,
            tzinfo=ZoneInfo("Asia/Kolkata")
        )
        ...
        logger.warning(f"check_punched_out_early failed for user {user.id}: {e}")
        return False
...
scheduler.add_job(
    lambda: asyncio.run(fetch_indian_holidays_task()),
...
async def get_task_analytics(
    ...
    for task in tasks:
        ...
        if isinstance(created, str):
            in_month = created.startswith(month)
        elif isinstance(created, datetime):
            in_month = created.strftime("%Y-%m") == month
...
async def update_user(
    ...
    has_edit    = perms.get("can_edit_users", False)

    if not is_admin and not is_own and not has_edit:
...
async def handle_attendance(
    ...
    """
    Punch in / punch out for the currently logged-in user.

    On punch-in:
      • Compares actual IST punch-in time against the user's
        punch_in_time + grace_time window.
      • Stores is_late=True permanently if the user is late.

    On punch-out:
      • Compares actual IST punch-out time against the user's
        configured punch_out_time.
      • Stores punched_out_early=True permanently if the user left early.
      • Calculates and stores duration_minutes.
    """
    today     = datetime.now(ZoneInfo("Asia/Kolkata")).date()
    today_str = today.isoformat()

    # Block on confirmed holidays
    ...
    if action == "punch_in":
        ...
        # ── Late detection ────────────────────────────────────────────────
        # Fetch fresh user document to guarantee up-to-date shift settings
        user_doc = await db.users.find_one({"id": current_user.id}, {"_id": 0})
        if user_doc:
            # Build a temporary User-like object just for the helper
            from types import SimpleNamespace
            _u = SimpleNamespace(
                id             = current_user.id,
                punch_in_time  = user_doc.get("punch_in_time",  "10:30"),
                grace_time     = user_doc.get("grace_time",     "00:15"),
                punch_out_time = user_doc.get("punch_out_time", "19:00"),
            )
        else:
            _u = current_user

        is_late = check_is_late(_u, punch_in_ist)
        # ─────────────────────────────────────────────────────────────────
        ...
        await db.attendance.update_one(
            {"user_id": current_user.id, "date": today_str},
            {
                "$set": {
                    ...
                    "leave_reason": None,
                    "is_late":      is_late,        # ← stored permanently
                }
            },
            upsert=True
        )
    ...
    if action == "punch_out":
        ...
        punched_out_early = check_punched_out_early(_u, punch_out_ist)
        # ─────────────────────────────────────────────────────────────────
        ...
        await db.attendance.update_one(
            {"user_id": current_user.id, "date": today_str},
            {
                "$set": {
                    ...
                    "punched_out_early": punched_out_early,  # ← stored permanently
                }
            }
        )
...
async def patch_task(
    ...
    is_authorized = (
        ...
        or is_own_record(current_user, existing_task)
    )
...
async def delete_task(task_id: str, current_user: User = Depends(get_current_user)):
    ...
    has_delete    = permissions.get("can_edit_tasks", False)

    if not (is_admin or is_creator or has_delete):
        raise HTTPException(
            status_code=403,
...
async def add_task_comment(
    ...
    is_involved = (
        current_user.role.lower() == "admin"
        or is_own_record(current_user, task)
    )
    if not is_involved:
...
async def update_dsc(
    ...
    if isinstance(updated["created_at"],  str): updated["created_at"]  = datetime.fromisoformat(updated["created_at"])
    if isinstance(updated["issue_date"],  str): updated["issue_date"]  = datetime.fromisoformat(updated["issue_date"])
    if isinstance(updated["expiry_date"], str): updated["expiry_date"] = datetime.fromisoformat(updated["expiry_date"])
    return DSC(**updated)
...
async def create_document(document_data: DocumentCreate, current_user: User = Depends(get_current_user)):
    ...
    doc["created_at"] = doc["created_at"].isoformat()
    if doc.get("issue_date"):  doc["issue_date"]  = doc["issue_date"].isoformat()
    if doc.get("valid_upto"):  doc["valid_upto"]   = doc["valid_upto"].isoformat()
...
async def update_document(
    ...
    if update_data.get("issue_date"): update_data["issue_date"] = update_data["issue_date"].isoformat()
    if update_data.get("valid_upto"): update_data["valid_upto"]  = update_data["valid_upto"].isoformat()
...
async def get_due_dates(current_user: User = Depends(get_current_user)):
    ...
    for dd in due_dates:
        if isinstance(dd.get("created_at"), str): dd["created_at"] = datetime.fromisoformat(dd["created_at"])
        if isinstance(dd.get("due_date"),   str): dd["due_date"]   = datetime.fromisoformat(dd["due_date"])
...
async def update_due_date(
    ...
    if isinstance(updated.get("due_date"),   str): updated["due_date"]   = datetime.fromisoformat(updated["due_date"])
...
async def create_client(payload: dict, current_user: User = Depends(get_current_user)):
    try:
        ...
        client_obj = Client(**client_data.model_dump(), created_by=current_user.id)
...
async def get_client(client_id: str, current_user: User = Depends(get_current_user)):
    ...
    if current_user.role != "admin":
        is_assigned   = c.get("assigned_to") == current_user.id
        permissions   = get_user_permissions(current_user)
        extra_clients = permissions.get("assigned_clients", [])
...
async def send_client_birthday_email(
    ...
    if not c:
        raise HTTPException(status_code=404, detail="Client not found")
    background_tasks.add_task(send_birthday_email, c["email"], c["company_name"])
...
async def parse_mds_excel_for_client_form(
    ...
    company_name = (
        company_info.get("Company Name") or company_info.get("company_name") or ""
    ).strip()

    raw_email = (
        company_info.get("Email Id") or company_info.get("Email") or company_info.get("email") or ""
    )
    email = clean_email(raw_email)

    raw_phone = (
        company_info.get("Phone") or company_info.get("Mobile") or company_info.get("Contact") or ""
    )
    phone = clean_phone(raw_phone)

    raw_doi = (
        company_info.get("Date of Incorporation") or company_info.get("Incorporation Date") or ""
    )
    birthday    = parse_date(raw_doi)
    client_type = detect_type(company_name)

    notes_lines = []
    for key, label in [("CIN", "CIN"), ("Registration Number", "Reg No"),
                        ("Registered Address", "Address"),
                        ("Authorised Capital (Rs)", "Authorised Capital"),
                        ("Paid up Capital (Rs)", "Paid-up Capital")]:
        val = company_info.get(key, "")
        if val and val not in ("-", "nan"):
            prefix = "₹" if "Capital" in label else ""
            notes_lines.append(f"{label}: {prefix}{val}")
...
async def get_dashboard_stats(current_user: User = Depends(get_current_user)):
    ...
    for c in clients:
        if c.get("birthday"):
            bday           = date.fromisoformat(c["birthday"]) if isinstance(c["birthday"], str) else c["birthday"]
            this_year_bday = bday.replace(year=today_date.year)
            if this_year_bday < today_date:
                this_year_bday = bday.replace(year=today_date.year + 1)
            if 0 <= (this_year_bday - today_date).days <= 7:
    ...
    if current_user.role != "staff":
        ...
        for u in users:
            user_tasks = [t for t in tasks if t.get("assigned_to") == u["id"]]
...
async def get_activity_summary(
    ...
    for activity in activities:
        ...
        if website:
            user_summary[uid]["websites"].setdefault(website, 0)
            user_summary[uid]["websites"][website] += duration

        user_summary[uid]["categories"].setdefault(category, 0)
    ...
    for uid, data in user_summary.items():
        ...
        data["productivity_percent"] = (
            (productive_duration / total_duration) * 100
            if total_duration > 0 else 0
        )

        intensity_map  = {"duration": total_duration, "productivity_percent": data["productivity_percent"]}
        radar_metrics  = {"productivity": data["productivity_percent"], "attendance": 75, "task_completion": 80}
        tool_chain     = {"user_id": uid, "top_apps": data.get("apps_list", [])[:3]}

        data["intensityMap"]  = intensity_map
        data["radarMetrics"]  = radar_metrics
        data["toolChainData"] = tool_chain
        result.append(data)
...
async def get_performance_rankings(
    ...
    if (
        ...
        and (datetime.now(timezone.utc) - rankings_cache_time[cache_key]).total_seconds() < 300
    ):
    ...
    for user in users:
        ...
        badge = (
            "⭐ Star Performer" if overall_score >= 95
            else "🏆 Top Performer" if overall_score >= 85
            else "Good Performer"
        )
