...
def get_user_permissions(current_user: User) -> dict:
    """
    Safely extract the permissions dict from a User object regardless of
    whether .permissions is a Pydantic model or a plain dict.
    """
...
def is_own_record(current_user: User, record: dict) -> bool:
    ...
    Returns True if the current user is the owner / assignee of the record.
    Checks: user_id, assigned_to, created_by, sub_assignees.
    """
    uid = current_user.id
    return (
...
async def get_task_analytics(
 ...
 # ── OWNERSHIP RULE: every user sees their own tasks; admin sees all ──
 query = {}
...
async def create_todo(
 ...
 # Any authenticated user can create their own todo (ownership rule)
 todo = Todo(
  user_id=current_user.id,
...
async def get_todos(
 ...
 # ── OWNERSHIP RULE: every user always sees their own todos ──
 if current_user.role == "admin":
    # Admin: all todos, optionally filtered by user_id
    query = {} if not user_id else {"user_id": user_id}
 elif current_user.role == "manager":
    team_ids = await get_team_user_ids(current_user.id)
    if user_id:
        # Manager can view their own or any team member's todos
        if user_id not in team_ids and user_id != current_user.id:
            raise HTTPException(status_code=403, detail="Not allowed")
        query = {"user_id": user_id}
    else:
        # Default: own todos + team todos
        query = {"user_id": {"$in": team_ids + [current_user.id]}}
 else:
    # Staff: own todos only, unless requesting own user_id explicitly
...
async def get_todo_dashboard(current_user: User = Depends(get_current_user)):
 ...
 # =========================
  ...
  team_ids = await get_team_user_ids(current_user.id)
  # Always include the manager's own todos
 ...
 # =========================
 # STAFF VIEW (OWN + EXPLICITLY ALLOWED)
 # =========================
 else:
  ...
  if not isinstance(allowed_users, list):
   allowed_users = []
  # Ownership rule: always include own todos
...
async def promote_todo(todo_id: str, current_user: User = Depends(get_current_user)):
 ...
 # Ownership rule: only the creator or admin can promote
...
async def delete_todo(
 ...
 if not todo:
  raise HTTPException(status_code=404, detail="Todo not found")
 # Ownership rule: only own todos or admin
 if current_user.role != "admin" and todo["user_id"] != current_user.id:
...
async def update_user(
    ...
    - Any user can update their own profile (ownership rule).
    """
    ...
    # Handle Password safely — any user can change their own password
...
async def handle_attendance(
    ...
    # Ownership rule: attendance is always for the current user themselves
...
async def mark_leave_today(current_user: User = Depends(get_current_user)):
    # Ownership rule: any user can mark their own leave
    today = datetime.now(ZoneInfo("Asia/Kolkata")).date()
...
async def get_today_attendance(current_user: User = Depends(get_current_user)):
    # Ownership rule: always returns the current user's own attendance
    today = datetime.now(ZoneInfo("Asia/Kolkata")).date()
    today_str = today.isoformat()
    # FIX: Query using today_str string
    holiday = await db.holidays.find_one({"date": today_str})
...
async def apply_leave(
 ...
 # Ownership rule: any user can apply leave for themselves
 try:
...
async def get_task_comments(
    ...
    # Ownership rule: admin or any involved party can view comments
...
async def create_tasks_bulk(
 ...
 # Any authenticated user can create tasks in bulk (ownership rule)
 created_tasks = []
...
async def get_tasks(current_user: User = Depends(get_current_user)):
 ...
 if current_user.role == "admin":
  ...
  team_ids = await get_team_user_ids(current_user.id)
  # Manager sees own tasks + team tasks (ownership rule covers manager's own)
  query["$or"] = [
   {"assigned_to": current_user.id},
  ...
  # OWNERSHIP RULE (Layer 4):
  # Staff always see tasks where they are assigned, sub-assignee, or creator.
  # Additionally, if admin has granted cross-user access via view_other_tasks,
  # those tasks are included too (Layer 3).
  permissions = get_user_permissions(current_user)
  allowed_users = permissions.get("view_other_tasks", [])
  query["$or"] = [
   {"assigned_to": current_user.id},      # own assigned tasks
   {"sub_assignees": current_user.id},    # tasks where user is sub-assignee
   {"created_by": current_user.id},       # tasks created by user
   {"assigned_to": {"$in": allowed_users}} # cross-user grant (Layer 3)
  ]
 tasks = await db.tasks.find(query, {"_id": 0}).to_list(1000)
 # Map user names
 user_ids = {
...
async def get_task(task_id: str, current_user: User = Depends(get_current_user)):
 ...
 if current_user.role == "admin":
  ...
  pass  # owner/assignee/creator always has access
 else:
  # Layer 3: check cross-user grants
  permissions = get_user_permissions(current_user)
  allowed_users = permissions.get("view_other_tasks", [])
...
async def patch_task(
 ...
 if not existing_task:
  raise HTTPException(status_code=404, detail="Task not found")
 # OWNERSHIP RULE (Layer 4): Admin, Creator, Assignee, or Sub-assignee can edit
 is_authorized = (
...
async def delete_task(
 ...
 is_admin = current_user.role.lower() == "admin"
 permissions = get_user_permissions(current_user)
 # OWNERSHIP RULE: creator can always delete their own task.
 # Others need explicit can_edit_tasks permission.
...
async def add_task_comment(
 ...
 if not task:
  raise HTTPException(status_code=404, detail="Task not found")
 # OWNERSHIP RULE: Only involved parties (admin, creator, assignee, sub-assignees) can comment
 is_involved = (
  current_user.role.lower() == "admin" or
...
async def get_my_attendance_summary(
 ...
 """
 OWNERSHIP RULE: every user can always view their own attendance summary.
 No permission flag required.
 """
 now = datetime.now(IST)
...
async def get_staff_attendance_report(
 ...
 # Admin or users with can_view_attendance can see the aggregated staff report.
 # Staff can still see their own data via /attendance/my-summary (ownership rule).
 if current_user.role != "admin":
  permissions = get_user_permissions(current_user)
...
async def export_attendance_pdf(
 ...
 # OWNERSHIP RULE: users can always export their own attendance PDF.
 # Others need can_view_attendance permission.
 if user_id != current_user.id:
  permissions = get_user_permissions(current_user)
...
async def get_efficiency_report(
 ...
 """
 OWNERSHIP RULE: every user can always view their own efficiency report.
 Viewing another user's report requires admin role or explicit view_other_reports grant.
 """
...
async def get_client(client_id: str, current_user: User = Depends(get_current_user)):
 ...
 # OWNERSHIP RULE: own assigned client is always accessible
 if current_user.role != "admin":
...
async def get_dashboard_stats(current_user: User = Depends(get_current_user)):
 ...
 now = datetime.now(IST)
 # OWNERSHIP RULE: task query always scoped to current user unless admin
 task_query = {}
 if current_user.role != "admin":
  permissions = get_user_permissions(current_user)
  if not permissions.get("can_view_all_tasks", False):
   allowed_users = permissions.get("view_other_tasks", [])
   # Always include own tasks (ownership rule)
   task_query["$or"] = [
    {"assigned_to": current_user.id},
    {"sub_assignees": current_user.id},
    {"created_by": current_user.id},
 ...
 # Client statistics — scoped to own clients (ownership rule)
 if current_user.role == "admin":
  client_query = {}
 elif current_user.role == "manager":
  team_ids = await get_team_user_ids(current_user.id)
 ...
 # Upcoming due dates (next 30 days) — scoped by department (ownership rule)
 ...
 # Team workload (tasks per user) — only for admin/manager
 team_workload = []
...
async def log_staff_activity(
    ...
    """Log staff desktop activity — any user can log their own activity"""
...
app.include_router(api_router)import os
import re
import csv
import uuid
import logging
