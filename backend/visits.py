...
Collections used:
    ...
    db.visit_comments  — comments on visits
...
class VisitCreate(BaseModel):
    client_id:          str
    client_name:        Optional[str] = None          # denormalised for speed
    assigned_to:        str                            # user performing the visit
    ...
    location:           Optional[str] = None           # free-text address / map link
    # recurrence
...
class VisitUpdate(BaseModel):
    ...
    outcome:            Optional[str] = None           # filled after visit
...
def _can_read_visit(current_user: User, visit_owner_id: str) -> bool:
    """Return True if current_user may read a visit belonging to visit_owner_id."""
...
def _can_write_visit(current_user: User, visit_owner_id: str) -> bool:
    """Return True if current_user may create/edit a visit for visit_owner_id."""
    if current_user.role == "admin":
        return True
    if current_user.id == visit_owner_id:
        return True
...
def _expand_recurrence(
    ...
    """
    Given a base visit document, generate child visit documents for every
    recurrence occurrence up to end_date_str (or 1 year max).
    The base visit is NOT included in the returned list.
    """
    ...
    while current <= end_max:
        ...
        # comments & outcome reset for each occurrence
        child["comments"]        = []
        child["outcome"]         = None
...
# ── SUMMARY / STATS for a user + month ───────────────────────────────────────
@router.get("/summary")
async def visit_summary(
    user_id: Optional[str] = Query(None),
    month:   Optional[str] = Query(None),     # YYYY-MM  — defaults to current month
    current_user: User = Depends(get_current_user),
):
    ...
    return {
        ...
        "completion_rate": (
            round(by_status.get("completed", 0) / total * 100, 1) if total else 0
        ),
    }


# ── UPCOMING (next N days) — dashboard widget ─────────────────────────────────
@router.get("/upcoming")
...
async def get_visit(
    ...
    # client info
    client = await db.clients.find_one({"id": visit["client_id"]}, {"_id": 0, "company_name": 1, "phone": 1, "email": 1})
...
async def delete_visit(
    ...
    if delete_recurrences and visit.get("parent_visit_id") is None:
        # delete all children spawned from this parent
